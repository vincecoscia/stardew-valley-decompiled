...
public class SoundsHelper : ISoundsHelper
{
	...
	public virtual bool PlayLocal(string cueName, GameLocation location, Vector2? position, int? pitch, SoundContext context, out ICue cue)
	{
		try
		{
			...
			this.SetPitch(cue, ((float?)pitch) ?? 1200f, pitch.HasValue);
			if (!this.ShouldPlayLocal(context))
			{
				this.LogSound?.Invoke(cueName, location, position, pitch, 1f, context, "disabled for context");
				return false;
			}
			...
			this.LogSound?.Invoke(cueName, location, position, pitch, volume, context, null);
			return true;
		}
		catch (Exception ex)
		{
	...
	public virtual void PlayAll(string cueName, GameLocation location, Vector2? position, int? pitch, SoundContext context)
	{
		...
		{
			this.PlayLocal(cueName, location, position, pitch, context, out var _);
		}
		else
		{
	...
	public virtual bool CanSkipSoundSync(GameLocation location, Vector2? position, SoundContext context)
	{
		...
		if (location != null)
		{
			...
			if (someoneCanHear && position.HasValue && position != Vector2.Zero)
			{
				...
				{
					if (!someoneCanHear && this.ShouldPlayLocal(context) && this.GetVolumeForDistance(location, position) > 0f)
					{
						someoneCanHear = true;
					}
				});
