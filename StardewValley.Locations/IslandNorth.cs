...
public class IslandNorth : IslandLocation
{
	...
	public override void TransferDataFromSavedLocation(GameLocation l)
	{
		...
		{
			this.bridgeFixed.Value = location.bridgeFixed;
	...
	{
		int index = base.getTileIndexAt(tileLocation.X, tileLocation.Y, "Buildings");
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if (!this.caveOpened && Utility.isOnScreen(Utility.PointToVector2(this.boulderPosition.Location), 1))
		{
		...
		if (!this._sawFlameSpriteNorth && Utility.isThereAFarmerWithinDistance(new Vector2(41f, 30f), 5, this) == Game1.player)
		{
			...
			TemporaryAnimatedSprite v = base.getTemporarySpriteByID(9999);
			if (v != null)
			{
				v.yPeriodic = false;
				v.xPeriodic = false;
			...
			base.localSound("magma_sprite_spot");
			v = base.getTemporarySpriteByID(9998);
			if (v != null)
			{
				v.yPeriodic = false;
				v.xPeriodic = false;
	...
	{
		if (!this.caveOpened && this.boulderPosition.Intersects(position))
		{
			return true;
		}
	...
	public override bool isTilePlaceable(Vector2 tile_location, bool itemIsPassable = false)
	{
		...
		if (!this.caveOpened && this.boulderPosition.Contains(non_tile_position))
		{
			return false;
		}
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		foreach (Vector2 v3 in base.terrainFeatures.Keys)
		{
			if (base.terrainFeatures[v3] is HoeDirt { crop: not null } dirt && dirt.crop.forageCrop.Value)
			{
				gingerTiles.Add(v3);
			}
		}
		...
		{
			base.terrainFeatures.Remove(v2);
		}
		...
		if (Utility.getNumObjectsOfIndexWithinRectangle(digSiteBounds, new string[9] { "(O)816", "(O)817", "(O)818", "(O)819", "(O)32", "(O)38", "(O)40", "(O)42", "(O)590" }, this) < 60)
		{
			for (int i = 0; i < numberOfAdditionsToTry; i++)
			{
				...
				else if (Game1.random.NextDouble() < 0.1)
				{
					...
					if (this.CanItemBePlacedHere(position, itemIsPassable: false, CollisionMask.All, CollisionMask.None) && !this.IsTileOccupiedBy(position) && base.getTileIndexAt(xCoord, yCoord, "AlwaysFront") == -1 && base.getTileIndexAt(xCoord, yCoord, "Front") == -1 && !base.isBehindBush(position) && this.doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") != null && this.doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") == "T")
					{
	...
	public override bool performOrePanTenMinuteUpdate(Random r)
	{
		if (Game1.MasterPlayer.mailReceived.Contains("ccFishTank") && base.orePanPoint.Value.Equals(Point.Zero) && r.NextBool())
		{
			for (int tries = 0; tries < 3; tries++)
			{
				...
				if (base.isOpenWater(p.X, p.Y) && FishingRod.distanceToLand(p.X, p.Y, this) <= 1 && base.getTileIndexAt(p, "Buildings") == -1)
				{
	...
	{
		if (!this.caveOpened && tileY == 47 && (tileX == 21 || tileX == 22))
		{
			this.boulderKnockTimer = 500f;
			Game1.playSound("hammer");
	...
	public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
	{
		...
		if (!this.caveOpened && this.boulderTextTimer > 0f)
		{
