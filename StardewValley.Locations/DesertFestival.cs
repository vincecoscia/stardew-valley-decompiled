...
public class DesertFestival : Desert
{
	...
	[XmlIgnore]
	public static int highestCalicoEggRatingToday = 0;
	...
	private bool gotPrizeFromGilToday;
	...
	public virtual void CollectRacePrizes()
	{
		...
		if (this.specialRewardsCollected.ContainsKey(Game1.player.UniqueMultiplayerID) && !this.specialRewardsCollected[Game1.player.UniqueMultiplayerID])
		{
		...
		if (full_action_string.Split(' ')[0] == "DesertMakeover")
		{
			...
			else
			{
				...
				Game1.globalFadeToBlack(delegate
				{
					...
					Event @event = new Event(this.GetMakeoverEvent());
	...
	public override bool performAction(string action, Farmer who, Location tile_location)
	{
		...
		case "DesertFishingBoard":
			if (!this.hasAcceptedFishingQuestToday)
			{
		...
		case "DesertRacerMan":
			...
			if (this.specialRewardsCollected.ContainsKey(Game1.player.UniqueMultiplayerID) && !this.specialRewardsCollected[Game1.player.UniqueMultiplayerID])
			{
				...
				Game1.afterDialogues = (Game1.afterFadeFunction)Delegate.Combine(Game1.afterDialogues, (Game1.afterFadeFunction)delegate
				{
					this.CollectRacePrizes();
				});
				return true;
			}
			if (this.rewardsToCollect.ContainsKey(Game1.player.UniqueMultiplayerID) && this.rewardsToCollect[Game1.player.UniqueMultiplayerID] > 0)
			{
			...
			{
				if (this.raceGuesses.ContainsKey(Game1.player.UniqueMultiplayerID) && this.currentRaceState.Value == RaceState.Go)
				{
					int guessed_racer = this.raceGuesses[Game1.player.UniqueMultiplayerID];
			...
			if (this.nextRaceGuesses.ContainsKey(Game1.player.UniqueMultiplayerID))
			{
				int guessed_racer2 = this.nextRaceGuesses[Game1.player.UniqueMultiplayerID];
		...
		case "DesertShadyGuy":
			...
			if (this.sabotages.ContainsKey(Game1.player.UniqueMultiplayerID))
			{
		...
		case "DesertGil":
			...
			if (DesertFestival.highestCalicoEggRatingToday == 0)
			{
				Game1.DrawDialogue((Game1.getLocationFromName("AdventureGuild") as AdventureGuild).Gil, "Strings\\1_6_Strings:Gil_NoRating");
				return true;
			}
			base.createQuestionDialogue(Game1.content.LoadString("Strings\\1_6_Strings:Gil_SubmitRating", DesertFestival.highestCalicoEggRatingToday + 1), base.createYesNoResponses(), "Gil_EggRating");
			return true;
		case "DesertMarlon":
		{
	...
	public virtual void ShowSabotagedRaceText()
	{
		if (this.sabotages.ContainsKey(Game1.player.UniqueMultiplayerID))
		{
			int sabotaged_racer = this.sabotages[Game1.player.UniqueMultiplayerID];
			if (this._localSabotageText == -1)
			{
				this._localSabotageText = Game1.random.Next(1, 4);
			}
			Game1.drawObjectDialogue(Game1.content.LoadString("Strings\\1_6_Strings:Shady_Guy_Selected_" + this._localSabotageText, Game1.content.LoadString("Strings\\1_6_Strings:Racer_" + sabotaged_racer)));
		}
	}
	...
	public override bool answerDialogueAction(string question_and_answer, string[] question_params)
	{
		...
		if (question_and_answer.Equals("Fishing_Quest_Yes"))
		{
			...
			this.hasAcceptedFishingQuestToday = true;
			return true;
		}
		...
		{
			this.gotPrizeFromGilToday = true;
			Game1.player.freezePause = 1400;
			DelayedAction.playSoundAfterDelay("coin", 500);
			DelayedAction.functionAfterDelay(delegate
			{
				int num = DesertFestival.highestCalicoEggRatingToday + 1;
				int eggPrize = 0;
		...
		if (question_and_answer.StartsWith("Cook"))
		{
			...
			else if (question_and_answer.StartsWith("Cook_PickedIngredient"))
			{
				...
				for (int j = 0; j < 5; j++)
				{
					if (j != this._cookIngredient || this._cookIngredient == 4)
					{
	...
	public virtual void AnnounceRace(string text)
	{
		...
		{
			this._raceTextTimer = 4f;
			if (this.raceGuesses.ContainsKey(Game1.player.UniqueMultiplayerID))
			{
				if (this.lastRaceWinner.Value == this.raceGuesses[Game1.player.UniqueMultiplayerID])
				{
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		this.gotPrizeFromGilToday = false;
		...
		DesertFestival.highestCalicoEggRatingToday = 0;
		this.hasAcceptedFishingQuestToday = false;
		this.finishedRacers.Clear();
	...
	public static void SetupFestivalDay()
	{
		...
		{
			NPC character4 = characters[l];
			if (!store_data_sheet.ContainsKey(festival_id + "_" + character4.Name) || (character4.Name == "Leo" && !Game1.MasterPlayer.mailReceived.Contains("leoMoved")))
			{
				characters.RemoveAt(l);
				l--;
			}
			else if (character4.getMasterScheduleRawData().ContainsKey(festival_id + "_" + day_number))
			{
				characters.RemoveAt(l);
				l--;
			}
