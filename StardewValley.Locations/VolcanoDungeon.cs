...
public class VolcanoDungeon : IslandLocation
{
	...
	public virtual void GenerateContents(bool use_level_level_as_layout = false)
	{
		...
		{
			this.generationSeed.Value = Utility.CreateRandomSeed(Game1.stats.DaysPlayed * this.level.Value, this.level.Value * 5152, Game1.uniqueIDForThisGame / 2);
			...
			default:
			{
				...
				if ((int)this.level > 1 && layout_random.NextDouble() < 0.5 * (double)luckMultiplier)
				{
					bool foundSpecialLevel = false;
					for (int l = 0; l < VolcanoDungeon.activeLevels.Count; l++)
					{
						if (VolcanoDungeon.activeLevels[l].layoutIndex.Value >= 32)
						{
				...
				{
					if (VolcanoDungeon.activeLevels[j].level.Value == this.level.Value - 1)
					{
						valid_layouts.Remove(VolcanoDungeon.activeLevels[j].layoutIndex.Value);
						break;
					}
				}
		...
		base.setMapTileIndex(36, 30, 77, "Buildings");
		base.setMapTileIndex(36, 29, 61, "Front");
		base.setTileProperty(36, 30, "Buildings", "Action", "asedf");
		base.setMapTileIndex(35, 31, 78, "Back");
		base.setMapTileIndex(36, 31, 79, "Back");
		base.setMapTileIndex(37, 31, 62, "Back");
		if (Game1.IsMasterGame)
		{
	...
	{
		if ((int)this.layoutIndex >= 32)
		{
			return (int)this.layoutIndex <= 34;
		}
		return false;
	}
	...
	{
		if ((int)this.layoutIndex >= 35)
		{
			return (int)this.layoutIndex <= 37;
		}
		return false;
	}
	...
	public virtual void UpdateShortcutOut()
	{
		if (this == Game1.currentLocation)
		{
			...
			{
				base.setMapTileIndex(VolcanoDungeon.shortcutOutPosition.X, VolcanoDungeon.shortcutOutPosition.Y, 367, "Buildings");
				base.removeTile(VolcanoDungeon.shortcutOutPosition.X, VolcanoDungeon.shortcutOutPosition.Y - 1, "Front");
			}
			else
			{
				base.setMapTileIndex(VolcanoDungeon.shortcutOutPosition.X, VolcanoDungeon.shortcutOutPosition.Y, 399, "Buildings");
				base.setMapTileIndex(VolcanoDungeon.shortcutOutPosition.X, VolcanoDungeon.shortcutOutPosition.Y - 1, 383, "Front");
			}
		}
	}
	...
	public virtual void UpdateBridge()
	{
		...
		{
			base.setMapTileIndex(27, 39, 399, "Buildings");
			base.setMapTileIndex(27, 38, 383, "Front");
		}
		...
		for (int x = 28; x <= 32; x++)
		{
			for (int y = 34; y <= 37; y++)
			{
				base.setMapTileIndex(x, y, x switch
				{
					28 => y switch
					{
						34 => 189, 
				...
				NPC i = base.isCharacterAtTile(new Vector2(x, y));
				if (i is Monster)
				{
					base.characters.Remove(i);
				}
	...
	public override string GetLocationSpecificMusic()
	{
		...
		if ((int)this.level == 1 || ((Game1.random.NextDouble() < 0.25 || (int)this.level == 6) && (Game1.getMusicTrackName() == "none" || Game1.isMusicContextActiveButNotPlaying() || Game1.getMusicTrackName().EndsWith("_Ambient"))))
		{
			return "VolcanoMines";
		}
		return "Volcano_Ambient";
	...
	{
		if ((int)this.level == 5 && new Microsoft.Xna.Framework.Rectangle(27, 29, 4, 4).Contains(tileX, tileY))
		{
			return true;
		}
		if ((int)this.level == 0 && tileX > 23 && tileX < 28 && tileY > 42 && tileY < 47)
		{
			return true;
		}
		return false;
	...
	public virtual void GenerateLevel(bool use_level_level_as_layout = false)
	{
		...
		base.mapPath.Value = "Maps\\Mines\\VolcanoTemplate";
		this.updateMap();
		base.loadedMapPath = base.mapPath;
		...
		{
			int j;
			for (j = 0; j < 32 && !(this.GetPixel(x + j, y, Color.Black) != set_piece_color) && !(this.GetPixel(x, y + j, Color.Black) != set_piece_color); j++)
			{
			}
			VolcanoDungeon.setPieceAreas.Add(new Microsoft.Xna.Framework.Rectangle(x, y, j, j));
			for (int k = 0; k < j; k++)
			{
				for (int l = 0; l < j; l++)
				{
					this.SetPixelMap(x + k, y + l, Color.White);
				}
			}
		});
		...
		this.ApplyToColor(Color.Blue, delegate(int x, int y)
		{
			...
			{
				base.sharedLights[x + y * 1000] = new LightSource(4, new Vector2(x, y) * 64f, 2f, new Color(0, 50, 50), x + y * 1000, LightSource.LightContext.None, 0L);
			}
		});
		...
		if ((int)this.level == 0)
		{
			...
			base.setMapTileIndex(40, 45, 266, "AlwaysFront");
			base.setMapTileIndex(40, 44, 76, "AlwaysFront");
			base.setMapTileIndex(39, 44, 76, "AlwaysFront");
			base.setMapTileIndex(41, 44, 76, "AlwaysFront");
			base.removeTile(40, 43, "Front");
			base.setMapTileIndex(40, 43, 70, "AlwaysFront");
			base.removeTile(39, 43, "Front");
			base.setMapTileIndex(39, 43, 69, "AlwaysFront");
			base.removeTile(41, 43, "Front");
			base.setMapTileIndex(41, 43, 69, "AlwaysFront");
			base.setMapTileIndex(39, 45, 265, "AlwaysFront");
			base.setMapTileIndex(41, 45, 267, "AlwaysFront");
			base.setMapTileIndex(40, 45, 60, "Back");
			base.setMapTileIndex(40, 46, 60, "Back");
			base.setMapTileIndex(40, 47, 60, "Back");
			base.setMapTileIndex(40, 48, 555, "Back");
			this.AddPossibleSwitchLocation(-1, 40, 51);
			this.CreateDwarfGate(-1, new Point(40, 48));
			base.setMapTileIndex(34, 30, 90, "Buildings");
			base.setMapTileIndex(34, 29, 148, "Buildings");
			base.setMapTileIndex(34, 31, 180, "Buildings");
			base.setMapTileIndex(34, 32, 196, "Buildings");
	...
	public virtual void CreateEntrance(Point? position)
	{
		...
		if (base.getTileIndexAt(position.Value.X - 1, position.Value.Y - 1, "Front") != -1)
		{
	...
	public virtual void ErodeInvalidDirtTiles()
	{
		...
		foreach (Point dirt_tile in this.dirtTiles)
		{
			...
			if (!fail && base.getTileIndexAt(dirt_tile, "Buildings") >= 0)
			{
				fail = true;
			}
			if (!fail)
	...
	public Point? GetDirtNeighborTile(int tile_x, int tile_y)
	{
		...
		if (base.getTileIndexAt(new Point(tile_x, tile_y), "Buildings") >= 0)
		{
			return null;
		}
	...
	{
		if ((int)this.level == 0 || (int)this.level == 5)
		{
			monsterChance = 0.0;
			itemChance = 0.0;
			gemStoneChance = 0.0;
	...
	public bool isTileClearForMineObjects(Vector2 v, bool ignoreRuins = false)
	{
		...
		if (ignoreRuins)
		{
			int tileIndex = base.getTileIndexAt((int)v.X, (int)v.Y, "Back");
	...
	public virtual void GenerateEntities()
	{
		...
		if ((int)this.level > 0 && (int)this.level != 5 && (this.generationRandom.NextBool() || this.isMushroomLevel()))
		{
			...
			for (int j = 0; j < numBarrels; j++)
			{
				...
				{
					p2 = new Point(this.generationRandom.Next(base.map.RequireLayer("Back").LayerWidth), 0);
					motion = new Point(0, 1);
				}
				else if (this.generationRandom.NextBool())
				{
					p2 = new Point(0, this.generationRandom.Next(base.map.RequireLayer("Back").LayerHeight));
					motion = new Point(1, 0);
				}
				else
				{
					p2 = new Point(base.map.RequireLayer("Back").LayerWidth - 1, this.generationRandom.Next(base.map.RequireLayer("Back").LayerHeight));
					motion = new Point(-1, 0);
				}
				while (base.isTileOnMap(p2.X, p2.Y))
				{
					p2.X += motion.X;
					p2.Y += motion.Y;
					if (this.isTileClearForMineObjects(new Vector2(p2.X, p2.Y)))
					{
						Vector2 objectPos2 = new Vector2(p2.X, p2.Y);
						if (this.isMushroomLevel())
						{
							base.terrainFeatures.Add(objectPos2, new CosmeticPlant(6 + this.generationRandom.Next(3)));
						}
						else
						{
		...
		if ((int)this.level != 5)
		{
			for (int x2 = 0; x2 < base.map.Layers[0].LayerWidth; x2++)
			{
				for (int y2 = 0; y2 < base.map.Layers[0].LayerHeight; y2++)
				{
					...
					if ((Math.Abs((float)this.startPosition.Value.X - objectPos.X) <= 5f && Math.Abs((float)this.startPosition.Value.Y - objectPos.Y) <= 5f) || (Math.Abs((float)this.endPosition.Value.X - objectPos.X) <= 5f && Math.Abs((float)this.endPosition.Value.Y - objectPos.Y) <= 5f))
					{
						continue;
					}
					if (this.CanItemBePlacedHere(objectPos) && this.generationRandom.NextDouble() < monsterChance)
					{
						if (base.getTileIndexAt((int)objectPos.X, (int)objectPos.Y, "Back") == 25)
						{
							if (!this.isMushroomLevel())
							{
								base.characters.Add(new Duggy(objectPos * 64f, magmaDuggy: true));
							}
						}
						else if (this.isMushroomLevel())
						{
							base.characters.Add(new RockCrab(objectPos * 64f, "False Magma Cap"));
						}
						else
						{
							base.characters.Add(new Bat(objectPos * 64f, ((int)this.level > 5 && this.generationRandom.NextBool()) ? (-556) : (-555)));
						}
					}
					else
					{
						if (!this.isTileClearForMineObjects(objectPos, ignoreRuins: true))
						{
							continue;
						}
		...
		foreach (Point p in spiker_spawn_points)
		{
			...
			{
				int direction = 1;
				switch (base.getTileIndexAt(p, "Back"))
				{
				case 537:
				case 538:
					direction = 2;
				...
				base.characters.Add(new Spiker(new Vector2(p.X, p.Y) * 64f, direction));
			}
		}
	}
	...
	private int chooseStoneTypeIndexOnly(Vector2 tile)
	{
		...
		float levelMod = 1f + (float)(int)this.level / 7f;
		...
		{
			if (base.objects.TryGetValue(v6, out var obj6) && (obj6.QualifiedItemId == "(O)843" || obj6.QualifiedItemId == "(O)844"))
			{
				chance += 0.15;
			}
		}
		...
		else
		{
			...
			{
				if (base.objects.TryGetValue(v5, out var obj5) && obj5.QualifiedItemId == "(O)765")
				{
					chance += 0.1;
				}
			}
			...
			else
			{
				...
				{
					if (base.objects.TryGetValue(v4, out var obj4) && obj4.QualifiedItemId == "(O)VolcanoGoldNode")
					{
						chance += 0.2;
					}
				}
				...
				else
				{
					...
					{
						if (base.objects.TryGetValue(v3, out var obj3) && obj3.QualifiedItemId.StartsWith("(O)VolcanoCoalNode"))
						{
							chance += 0.2;
						}
					}
					...
					else
					{
						...
						{
							if (base.objects.TryGetValue(v2, out var obj2) && obj2.QualifiedItemId == "(O)850")
							{
								chance += 0.25;
							}
						}
						...
						else
						{
							...
							{
								if (base.objects.TryGetValue(v, out var obj) && obj.QualifiedItemId == "(O)849")
								{
									chance += 0.25;
								}
							}
		...
		if ((int)this.level > 2 && this.generationRandom.NextDouble() < 0.0002)
		{
			whichStone = 46;
		}
		return whichStone;
	...
	{
		if ((int)this.level != 5 && t is WateringCan && base.isTileOnMap(new Vector2(tileX, tileY)) && base.waterTiles[tileX, tileY] && !this.cooledLavaTiles.ContainsKey(new Vector2(tileX, tileY)))
		{
	...
	public virtual void GenerateWalls(Color match, int source_x, int source_y, int wall_height = 4, int random_wall_variants = 1, bool start_in_wall = false, Action<int, int> on_insufficient_wall_height = null, bool use_corner_hack = false)
	{
		...
		for (int pass = 0; pass < 2; pass++)
		{
			for (int x2 = 0; x2 < this.mapWidth; x2++)
			{
				...
				{
					if (this.GetPixel(x2, current_y, match) != match || current_y >= this.mapHeight)
					{
						...
						for (int curr_y = current_y - 1; curr_y > last_y; curr_y--)
						{
							if (clearance < wall_height)
							{
								...
								{
									on_insufficient_wall_height(x2, curr_y);
								}
								else
								{
									this.SetPixelMap(x2, curr_y, Color.White);
									this.PlaceSingleWall(x2, curr_y);
								}
								current_height--;
							}
							else
							{
								...
								{
								case 0:
									if (this.GetPixelClearance(x2 - 1, curr_y, wall_height, match) < wall_height && this.GetPixelClearance(x2 + 1, curr_y, wall_height, match) < wall_height)
									{
										if (on_insufficient_wall_height != null)
										{
											on_insufficient_wall_height(x2, curr_y);
									...
									break;
								case 1:
									this.heightMap[x2 + curr_y * this.mapWidth] = current_height + 1;
									if (current_height < wall_height || wall_height == 0)
									{
										if (wall_height > 0)
										{
											this.SetTile(this.buildingsLayer, x2, curr_y, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants + wall_variant_index, source_y + 1 + random_wall_variants + wall_height - current_height - 1));
										}
									}
									else
									{
										this.SetTile(this.buildingsLayer, x2, curr_y, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants * 3, source_y));
									}
									break;
								}
							}
		...
		for (int y = 0; y < this.mapHeight; y++)
		{
			for (int x = 0; x < this.mapWidth; x++)
			{
				int height = this.GetHeight(x, y, wall_height);
				int left_height = this.GetHeight(x - 1, y, wall_height);
				int right_height = this.GetHeight(x + 1, y, wall_height);
				int top_height = this.GetHeight(x, y - 1, wall_height);
				...
				if (right_height < height)
				{
					if (right_height == wall_height)
					{
						if (use_corner_hack)
						{
							corner_tiles.Add(new Point(x, y));
					...
					else
					{
						...
						if (right_height >= 0)
						{
							this.SetTile(this.buildingsLayer, x, y, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants, source_y + 1 + random_wall_variants + wall_height - right_height));
							target_layer = this.frontLayer;
						}
						if (height > wall_height)
						{
							this.SetTile(target_layer, x, y, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants * 3 - 1, source_y + 1 + index));
						}
						else
						{
							this.SetTile(target_layer, x, y, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants * 2 + index, source_y + 1 + random_wall_variants * 2 + 1 - height - 1));
						}
						if (wall_height > 0 && y + 1 < this.mapHeight && right_height == -1 && this.GetHeight(x + 1, y + 1, wall_height) >= 0 && this.GetHeight(x, y + 1, wall_height) >= 0)
						{
							if (use_corner_hack)
							{
								corner_tiles.Add(new Point(x, y));
				...
				else if (left_height < height)
				{
					...
					else
					{
						...
						if (left_height >= 0)
						{
							this.SetTile(this.buildingsLayer, x, y, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants, source_y + 1 + random_wall_variants + wall_height - left_height));
							target_layer2 = this.frontLayer;
						}
						if (height > wall_height)
						{
							this.SetTile(target_layer2, x, y, VolcanoDungeon.GetTileIndex(source_x, source_y + 1 + index));
						}
						else
						{
							this.SetTile(target_layer2, x, y, VolcanoDungeon.GetTileIndex(source_x + index, source_y + 1 + random_wall_variants * 2 + 1 - height - 1));
						}
						if (wall_height > 0 && y + 1 < this.mapHeight && left_height == -1 && this.GetHeight(x - 1, y + 1, wall_height) >= 0 && this.GetHeight(x, y + 1, wall_height) >= 0)
						{
							if (use_corner_hack)
							{
								corner_tiles.Add(new Point(x, y));
				...
				if (wall_height > 0)
				{
					if (right_height == -1)
					{
						this.SetTile(this.frontLayer, x, y - 1, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants * 2 + index, source_y));
					}
					else if (left_height == -1)
					{
						this.SetTile(this.frontLayer, x, y - 1, VolcanoDungeon.GetTileIndex(source_x + index, source_y));
					}
					else
					{
						this.SetTile(this.frontLayer, x, y - 1, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants + index, source_y));
					}
				}
				else if (right_height == -1)
				{
					this.SetTile(this.buildingsLayer, x, y, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants * 2 + index, source_y));
				}
				else if (left_height == -1)
				{
					this.SetTile(this.buildingsLayer, x, y, VolcanoDungeon.GetTileIndex(source_x + index, source_y));
				}
				else
				{
					this.SetTile(this.buildingsLayer, x, y, VolcanoDungeon.GetTileIndex(source_x + random_wall_variants + index, source_y));
				}
			}
		}
		if (use_corner_hack)
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if ((int)this.level == 0 && Game1.currentLocation == this)
		{
	...
	public override void drawWaterTile(SpriteBatch b, int x, int y)
	{
		...
		if ((int)this.level == 0 && x > 23 && x < 28 && y > 42 && y < 47)
		{
	...
	public override void performTenMinuteUpdate(int timeOfDay)
	{
		...
		if (!(Game1.random.NextDouble() < 0.1) || (int)this.level <= 0 || (int)this.level == 5)
		{
			return;
		}
		int numsprites = 0;
	...
	public void spawnFlyingMonsterOffScreen()
	{
		...
		base.characters.Add(new Bat(spawnLocation, ((int)this.level > 5 && Game1.random.NextBool()) ? (-556) : (-555))
		{
	...
	{
		if (base.getTileIndexAt(tileLocation.X, tileLocation.Y, "Buildings") == 367)
		{
		...
		if (base.getTileIndexAt(tileLocation.X, tileLocation.Y, "Buildings") == 77)
		{
