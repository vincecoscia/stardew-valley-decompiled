...
public static class TriggerActionManager
{
	...
	public static class DefaultActions
	{
		...
		public static bool If(string[] args, TriggerActionContext context, out string error)
		{
			...
			{
				string action2 = ArgUtility.UnsplitQuoteAware(ArgUtility.GetSubsetOf(args, startFalseIndex), ' ');
	...
	{
		TriggerActionManager.InitializeIfNeeded();
		if (TriggerActionManager.ActionHandlers.ContainsKey(name))
		{
			Game1.log.Warn("Warning: event command " + name + " is already defined and will be overwritten.");
			return;
		}
		TriggerActionManager.ActionHandlers[name] = action;
		...
		if (TriggerActionManager.ValidTriggerTypes.TryGetValue(trigger, out var actualTrigger))
		{
			...
			triggerArgs = triggerArgs ?? LegacyShims.EmptyArray<object>();
			{
				foreach (CachedTriggerAction entry in TriggerActionManager.GetActionsForTrigger(trigger))
				{
					...
					if (!TriggerActionManager.CanApply(data, location, player, targetItem, inputItem))
					{
						continue;
					}
	...
	{
		if (TriggerActionManager.ActionsByTrigger.TryGetValue(trigger, out var cached))
		{
			return cached;
		}
	...
	{
		if (TriggerActionManager.ActionHandlers.Count == 0)
		{
		...
		if (TriggerActionManager.ActionsByTrigger.Count != 0)
		{
			return;
		}
		foreach (string triggerType in TriggerActionManager.ValidTriggerTypes)
		{
			TriggerActionManager.ActionsByTrigger[triggerType] = new List<CachedTriggerAction>();
		}
		...
		foreach (TriggerActionData data in DataLoader.TriggerActions(Game1.content))
		{
			...
			if (data.Action != null)
			{
				CachedAction parsed2 = TriggerActionManager.ParseAction(data.Action);
			...
			if (data.Actions != null)
			{
				...
				{
					CachedAction parsed = TriggerActionManager.ParseAction(action2);
			...
			foreach (string trigger in array)
			{
				...
				{
					TriggerActionManager.ActionsByTrigger[trigger].Add(cachedTriggerAction);
				}
			}
		}
	}
