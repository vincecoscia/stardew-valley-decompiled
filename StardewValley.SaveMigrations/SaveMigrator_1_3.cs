...
public class SaveMigrator_1_3 : ISaveMigrator
{
	...
	private static void MigrateHorseIds()
	{
		...
		{
			Stable stable3 = stablesMissingHorses[j];
			GameLocation parentLocation = stable3.GetParentLocation();
			Rectangle boundingBox = stable3.GetBoundingBox();
			foreach (NPC character in parentLocation.characters)
			{
				if (character is Horse horse2 && horse2.HorseId == Guid.Empty && boundingBox.Intersects(horse2.GetBoundingBox()))
				{
		...
		{
			Stable stable2 = stablesMissingHorses[i];
