...
public class Toolbar : IClickableMenu
{
	...
	public override void draw(SpriteBatch b)
	{
		...
		for (int j = 0; j < 12; j++)
		{
			Vector2 toDraw2 = new Vector2(Game1.uiViewport.Width / 2 - 384 + j * 64, this.yPositionOnScreen - 96 + 8);
			b.Draw(Game1.menuTexture, toDraw2, Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, (Game1.player.CurrentToolIndex == j) ? 56 : 10), Color.White * this.transparency);
			if (!Game1.options.gamepadControls)
			{
				b.DrawString(Game1.tinyFont, this.slotText[j], toDraw2 + new Vector2(4f, -8f), Color.DimGray * this.transparency);
			}
		}
		for (int i = 0; i < 12; i++)
		{
			this.buttons[i].scale = Math.Max(1f, this.buttons[i].scale - 0.025f);
			Vector2 toDraw = new Vector2(Game1.uiViewport.Width / 2 - 384 + i * 64, this.yPositionOnScreen - 96 + 8);
			if (Game1.player.Items.Count > i && Game1.player.Items[i] != null)
			{
				Game1.player.Items[i].drawInMenu(b, toDraw, (Game1.player.CurrentToolIndex == i) ? 0.9f : (this.buttons[i].scale * 0.8f), this.transparency, 0.88f);
			}
		}
