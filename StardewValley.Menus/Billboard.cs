...
public class Billboard : IClickableMenu
{
	...
	public virtual List<BillboardEvent> GetEventsForDay(int day, Dictionary<int, List<NPC>> birthdays)
	{
		...
		{
			string festivalName2 = Game1.content.LoadString("Strings\\1_6_Strings:SquidFest");
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (!this.dailyQuestBoard)
		{
			...
			for (int j = 0; j < this.calendarDays.Count; j++)
			{
				ClickableTextureComponent component = this.calendarDays[j];
				if (this.calendarDayData.TryGetValue(component.myID, out var day))
				{
					...
					{
						Utility.drawWithShadow(b, Game1.mouseCursors_1_6, new Vector2(this.calendarDays[j].bounds.X + 8, (float)(this.calendarDays[j].bounds.Y + 60) - Game1.dialogueButtonScale / 2f), new Rectangle(103, 2, 10, 11), Color.White, 0f, Vector2.Zero, 4f, flipped: false, 1f);
					}
					...
					{
						b.Draw(Game1.mouseCursors_1_6, new Vector2((float)(component.bounds.Right - 72) - 2f * (float)Math.Sin((Game1.currentGameTime.TotalGameTime.TotalSeconds + (double)j * 0.3) * 3.0), (float)(component.bounds.Top + 52) - 2f * (float)Math.Cos((Game1.currentGameTime.TotalGameTime.TotalSeconds + (double)j * 0.3) * 2.0)), new Rectangle(71, 63, 8, 15), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
					}
				}
				if (Game1.dayOfMonth > j + 1)
				{
					b.Draw(Game1.staminaRect, component.bounds, Color.Gray * 0.25f);
				}
				else if (Game1.dayOfMonth == j + 1)
				{
		...
		else
		{
			...
			else
			{
				...
				if (Game1.stats.Get("BillboardQuestsDone") % 3 == 2 && (this.acceptQuestButton.visible || !Game1.questOfTheDay.completed))
				{
			...
			{
				b.Draw(this.billboardTexture, base.Position + new Vector2(18 + 12 * i, 36f) * 4f, new Rectangle(140, 397, 10, 11), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.6f);
			}
