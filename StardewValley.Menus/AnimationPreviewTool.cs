...
public class AnimationPreviewTool : IClickableMenu
{
	...
	{
		foreach (ClickableTextureComponent component2 in this.animationButtons)
		{
			if (component2.bounds.Contains(x, y) && this.scrollView.Contains(x, y))
			{
		...
		foreach (List<ClickableTextureComponent> component3 in this.components)
		{
			foreach (ClickableTextureComponent component in component3)
			{
				if (component.containsPoint(x, y))
				{
					...
					{
					case "Shirt Style":
						this.SwitchShirt(component.myID);
						break;
					case "Pants Style":
						this.SwitchPants(component.myID);
						break;
					case "Hair Style":
						this.SwitchHair(component.myID);
						break;
					case "Toggle Gender":
