...
public class MapPage : IClickableMenu
{
		...
		: base(x, y, width, height)
	{
		...
		this.mapRegion = this.mapPosition.Region;
		this.mapAreas = this.mapRegion.GetAreas();
		this.scrollText = this.mapPosition.GetScrollText(playerTile);
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				Rectangle pixelArea = tooltip2.GetPixelArea();
				pixelArea = new Rectangle(this.mapBounds.X + pixelArea.X, this.mapBounds.Y + pixelArea.Y, pixelArea.Width, pixelArea.Height);
				id++;
				ClickableComponent component2 = new ClickableComponent(pixelArea, tooltip2.NamespacedId)
				{
					myID = id,
					label = tooltip2.Text
				};
				this.points[tooltip2.NamespacedId] = component2;
				if (tooltip2.NamespacedId == "Farm/Default")
				{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				if (this.points.TryGetValue(tooltip.NamespacedId, out var component))
				{
	...
	public virtual void drawMiniPortraits(SpriteBatch b)
	{
		...
		foreach (Farmer player in Game1.getOnlineFarmers())
		{
			...
			MapAreaPosition positionData = (player.IsLocalPlayer ? this.mapPosition : WorldMapManager.GetPositionData(player.currentLocation, tile));
			if (positionData != null && !(positionData.Region.Id != this.mapRegion.Id))
			{
	...
	public virtual void drawMap(SpriteBatch b, bool drawBorders = true)
	{
		...
		if (baseTexture != null)
		{
			Rectangle destRect2 = baseTexture.GetOffsetMapPixelArea(this.mapBounds.X, this.mapBounds.Y);
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				Rectangle destRect = overlay.GetOffsetMapPixelArea(this.mapBounds.X, this.mapBounds.Y);
