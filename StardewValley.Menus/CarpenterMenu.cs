...
public class CarpenterMenu : IClickableMenu
{
	...
	public class BlueprintEntry
	{
		...
		public void SetSkin(string id)
		{
			...
			this.DisplayName = TokenParser.ParseText(this.Data.Name);
	...
	public override void performHoverAction(int x, int y)
	{
		...
		else
		{
			...
			if (this.upgrading)
			{
				if (b != null)
				{
					b.color = ((b.buildingType == this.Blueprint.UpgradeFrom) ? (Color.Lime * 0.8f) : (Color.Red * 0.8f));
				}
			}
			...
			else if (this.painting && b != null && (b.CanBePainted() || b.CanBeReskinned(ignoreSeparateConstructionEntries: true)) && this.HasPermissionsToPaint(b))
			{
				b.color = Color.Lime * 0.8f;
			}
		}
	...
	protected bool VerifyTileAccessibility(int tileX, int tileY, Vector2 buildingPosition)
	{
		...
		if (!this.buildingToMove.isTilePassable(new Vector2((int)this.buildingToMove.tileX + relativeX, (int)this.buildingToMove.tileY + relativeY)))
		{
			return false;
		}
	...
	public virtual bool ConfirmBuildingAccessibility(Vector2 buildingPosition)
	{
		...
		if (this.buildingToMove.buildingType != "Farmhouse")
		{
			return true;
		}
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		if (!this.onFarm)
		{
			...
			if (this.moveButton.containsPoint(x, y) && this.moveButton.visible)
			{
				Game1.globalFadeToBlack(setUpForBuildingPlacement);
				Game1.playSound("smallSelect");
				this.onFarm = true;
			...
			if (this.paintButton.containsPoint(x, y) && this.paintButton.visible)
			{
				Game1.globalFadeToBlack(setUpForBuildingPlacement);
				Game1.playSound("smallSelect");
				this.onFarm = true;
			...
			if (this.appearanceButton.containsPoint(x, y) && this.appearanceButton.visible && this.currentBuilding.CanBeReskinned(ignoreSeparateConstructionEntries: true))
			{
			...
			if (this.okButton.containsPoint(x, y) && !this.onFarm && this.CanBuildCurrentBlueprint())
			{
				Game1.globalFadeToBlack(setUpForBuildingPlacement);
				Game1.playSound("smallSelect");
				this.onFarm = true;
		...
		if (this.upgrading)
		{
			...
			if (toUpgrade != null && toUpgrade.buildingType == this.Blueprint.UpgradeFrom)
			{
				...
				this.freeze = true;
				Game1.multiplayer.globalChatInfoMessage("BuildingBuild", Game1.player.Name, "aOrAn:" + this.Blueprint.TokenizedDisplayName, this.Blueprint.TokenizedDisplayName, Game1.player.farmName);
		...
		if (this.moving)
		{
			if (this.buildingToMove == null)
			{
				...
				if (this.buildingToMove != null)
				{
					if ((int)this.buildingToMove.daysOfConstructionLeft > 0)
					{
	...
	public void robinConstructionMessage()
	{
		...
		string displayName = ((LocalizedContentManager.CurrentLanguageCode == LocalizedContentManager.LanguageCode.de) ? this.Blueprint.DisplayName : this.Blueprint.DisplayName.ToLower());
		...
		string[] displayNameParts = ArgUtility.SplitBySpace(this.Blueprint.DisplayName);
		string namePart;
	...
	public void setUpForBuildingPlacement()
	{
		...
		if (!this.demolishing && this.Blueprint.IsUpgrade && !this.moving && !this.painting)
		{
			this.upgrading = true;
		}
	}
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (!this.onFarm)
		{
			...
			this.currentBuilding.drawInMenu(b, base.xPositionOnScreen + this.maxWidthOfBuildingViewer / 2 - (int)this.currentBuilding.tilesWide * 64 / 2 - 64 + offset.X, base.yPositionOnScreen + this.maxHeightOfBuildingViewer / 2 - sourceRect.Height * 4 / 2 + offset.Y);
			b.End();
		...
		{
			string message = (this.upgrading ? Game1.content.LoadString("Strings\\UI:Carpenter_SelectBuilding_Upgrade", blueprint.GetDisplayNameForBuildingToUpgrade()) : (this.demolishing ? Game1.content.LoadString("Strings\\UI:Carpenter_SelectBuilding_Demolish") : ((!this.painting) ? Game1.content.LoadString("Strings\\UI:Carpenter_ChooseLocation") : Game1.content.LoadString("Strings\\UI:Carpenter_SelectBuilding_Paint"))));
			...
			if (!this.upgrading && !this.demolishing && !this.moving && !this.painting)
			{
				...
				for (int y3 = 0; y3 < this.currentBuilding.tilesHigh.Value; y3++)
				{
					for (int x3 = 0; x3 < this.currentBuilding.tilesWide.Value; x3++)
					{
						int sheetIndex2 = this.currentBuilding.getTileSheetIndexForStructurePlacementTile(x3, y3);
				...
				foreach (BuildingPlacementTile additionalPlacementTile in this.currentBuilding.GetAdditionalPlacementTiles())
				{
					...
					foreach (Point point in additionalPlacementTile.TileArea.GetPoints())
					{
						int x2 = point.X;
						int y2 = point.Y;
						int sheetIndex = this.currentBuilding.getTileSheetIndexForStructurePlacementTile(x2, y2);
			...
			else if (!this.painting && this.moving && this.buildingToMove != null)
			{
				...
				for (int y = 0; y < (int)this.buildingToMove.tilesHigh; y++)
				{
