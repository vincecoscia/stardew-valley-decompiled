...
public class SkillsPage : IClickableMenu
{
		...
		: base(x, y, width, height)
	{
		...
		for (int k = 4; k < 10; k += 5)
		{
			for (int l = 0; l < 5; l++)
			{
				...
				if (drawRed && (k + 1) % 5 == 0)
				{
					this.skillBars.Add(new ClickableTextureComponent(professionNumber.ToString() ?? "", new Rectangle(addedX + drawX - 4 + k * 36, drawY + l * 68, 56, 36), null, professionBlurb, Game1.mouseCursors, new Rectangle(159, 338, 14, 9), 4f, drawShadow: true)
					{
		...
		for (int j = 0; j < this.skillBars.Count; j++)
		{
			if (j < this.skillBars.Count - 1 && Math.Abs(this.skillBars[j + 1].myID - this.skillBars[j].myID) < 50)
			{
				this.skillBars[j].downNeighborID = this.skillBars[j + 1].myID;
				this.skillBars[j + 1].upNeighborID = this.skillBars[j].myID;
			}
		}
		...
		for (int i = 0; i < 5; i++)
		{
			...
			this.skillAreas.Add(new ClickableTextureComponent(index.ToString() ?? "", new Rectangle(drawX - 128 - 48, drawY + i * 68, 148, 36), index.ToString() ?? "", text, null, Rectangle.Empty, 1f)
			{
				myID = i,
	...
	public override void performHoverAction(int x, int y)
	{
		...
		foreach (ClickableComponent c3 in this.ccTrackerButtons)
		{
			if (c3 != null && c3.containsPoint(x, y))
			{
				this.hoverText = c3.label;
				break;
			}
		}
		...
		foreach (ClickableTextureComponent c in this.skillAreas)
		{
			if (c.containsPoint(x, y) && c.hoverText.Length > 0)
			{
				this.hoverText = c.hoverText;
				this.hoverTitle = Farmer.getSkillDisplayNameFromIndex(Convert.ToInt32(c.name));
				break;
			}
		}
	...
	public override void draw(SpriteBatch b)
	{
		...
		Game1.player.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(Game1.player.bathingClothes ? 108 : this.playerPanelFrames[this.playerPanelIndex], 0, secondaryArm: false, flip: false), Game1.player.bathingClothes ? 108 : this.playerPanelFrames[this.playerPanelIndex], new Rectangle(this.playerPanelFrames[this.playerPanelIndex] * 16, Game1.player.bathingClothes ? 576 : 0, 16, 32), new Vector2(x + 32, y + 16 - 4), Vector2.Zero, 0.8f, 2, Color.White, 0f, 1f, Game1.player);
