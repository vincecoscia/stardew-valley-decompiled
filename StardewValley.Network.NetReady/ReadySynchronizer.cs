...
public class ReadySynchronizer
{
	...
	{
		return this.GetIfExists(id)?.IsReady ?? false;
	}
	...
	{
		return this.GetIfExists(id)?.IsCancelable ?? false;
	}
	...
	{
		return this.GetIfExists(id)?.NumberReady ?? 0;
	}
	...
	{
		return this.GetIfExists(id)?.NumberRequired ?? 0;
	}

	/// <summary>Update all ready checks.</summary>
	public void Update()
	{
		foreach (BaseReadyCheck value in this.ReadyChecks.Values)
		{
			value.Update();
		}
	}
	...
	{
		if (id == null || !this.ReadyChecks.TryGetValue(id, out var check))
		{
			return null;
		}
		return check;
	...
	{
		if (this.ReadyChecks.TryGetValue(id, out var check))
		{
			return check;
		}
