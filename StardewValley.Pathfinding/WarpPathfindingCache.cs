...
public static class WarpPathfindingCache
{
	...
	private static void ExploreWarpPoints(string locationName, List<string> route, Gender? genderRestriction, HashSet<string> seenTargets)
	{
		...
		if (seenTargets.Add(locationName) && !WarpPathfindingCache.IgnoreLocationNames.Contains(locationName) && !MineShaft.IsGeneratedLevel(locationName, out var level) && !VolcanoDungeon.IsGeneratedLevel(locationName, out level))
		{
