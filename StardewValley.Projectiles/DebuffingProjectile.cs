...
public class DebuffingProjectile : Projectile
{
	...
	{
		if (!base.damagesMonsters && Game1.random.Next(11) >= player.Immunity && !player.hasBuff("28") && !player.hasTrinketWithID("BasiliskPaw"))
		{
	...
	{
		if ((bool)base.damagesMonsters && n is Monster && this.debuff == "frozen")
		{
			if ((int)(n as Monster).stunTime < 51)
			{
				base.piercesLeft.Value--;
			}
			if ((int)(n as Monster).stunTime < this.debuffIntensity.Value - 1000)
			{
				location.playSound("frozen");
