...
public class Ring : Item
{
		...
		: this()
	{
		...
		this.uniqueID.Value = Game1.year + Game1.dayOfMonth + Game1.timeOfDay + seedOffset + Game1.player.TilePoint.X + (int)Game1.stats.MonstersKilled + (int)Game1.stats.ItemsCrafted;
		this.loadDisplayFields();
	}
	...
	{
		base.initNetFields();
		base.NetFields.AddField(this.price, "price").AddField(this.uniqueID, "uniqueID");
	}
	...
	public override void onEquip(Farmer who)
	{
		...
		if (this._lightSourceID.HasValue)
		{
			...
			this._lightSourceID = null;
		}
		switch (base.ItemId)
		{
		case "516":
			this._lightSourceID = (int)this.uniqueID + (int)who.UniqueMultiplayerID;
			while (location.sharedLights.ContainsKey(this._lightSourceID.Value))
			{
				this._lightSourceID = this._lightSourceID.Value + 1;
			}
			location.sharedLights[this._lightSourceID.Value] = new LightSource(1, new Vector2(who.Position.X + 21f, who.Position.Y + 64f), 5f, new Color(0, 50, 170), (int)this.uniqueID + (int)who.UniqueMultiplayerID, LightSource.LightContext.None, who.UniqueMultiplayerID);
			break;
		case "517":
			this._lightSourceID = (int)this.uniqueID + (int)who.UniqueMultiplayerID;
			while (location.sharedLights.ContainsKey(this._lightSourceID.Value))
			...
			location.sharedLights[this._lightSourceID.Value] = new LightSource(1, new Vector2(who.Position.X + 21f, who.Position.Y + 64f), 10f, new Color(0, 30, 150), (int)this.uniqueID + (int)who.UniqueMultiplayerID, LightSource.LightContext.None, who.UniqueMultiplayerID);
			break;
		case "888":
			this._lightSourceID = (int)this.uniqueID + (int)who.UniqueMultiplayerID;
			while (location.sharedLights.ContainsKey(this._lightSourceID.Value))
			...
			location.sharedLights[this._lightSourceID.Value] = new LightSource(1, new Vector2(who.Position.X + 21f, who.Position.Y + 64f), 10f, new Color(0, 80, 0), (int)this.uniqueID + (int)who.UniqueMultiplayerID, LightSource.LightContext.None, who.UniqueMultiplayerID);
			break;
		case "527":
			this._lightSourceID = (int)this.uniqueID + (int)who.UniqueMultiplayerID;
			while (location.sharedLights.ContainsKey(this._lightSourceID.Value))
	...
	public override void onUnequip(Farmer who)
	{
		...
		case "527":
			if (this._lightSourceID.HasValue)
			{
				who.currentLocation.removeLightSource(this._lightSourceID.Value);
				this._lightSourceID = null;
			}
			break;
		}
	...
	public virtual void onNewLocation(Farmer who, GameLocation environment)
	{
		...
		case "527":
			...
			environment.sharedLights[this._lightSourceID.Value] = new LightSource(1, new Vector2(who.Position.X + 21f, who.Position.Y + 64f), 10f, new Color(0, 30, 150), LightSource.LightContext.None, who.UniqueMultiplayerID);
			break;
		}
	}
	...
	public virtual void onLeaveLocation(Farmer who, GameLocation environment)
	{
		...
		case "527":
		case "888":
			if (!this._lightSourceID.HasValue && who != null)
			{
				this._lightSourceID = (int)this.uniqueID + (int)who.UniqueMultiplayerID;
				bool found = false;
				while (environment.sharedLights.ContainsKey(this._lightSourceID.Value))
	...
	public virtual void onMonsterSlay(Monster monster, GameLocation location, Farmer who)
	{
		...
		{
			location?.explode(monster.Tile, 2, who, damageFarmers: false);
		}
	...
	public virtual void update(GameTime time, GameLocation environment, Farmer who)
	{
		...
		environment.repositionLightSource(this._lightSourceID.Value, new Vector2(who.Position.X + 21f, who.Position.Y) + offset);
		if (!environment.isOutdoors && !(environment is MineShaft) && !(environment is VolcanoDungeon))
		{
			LightSource i = environment.getLightSource(this._lightSourceID.Value);
			if (i != null)
			{
				i.radius.Value = 3f;
			}
	...
	public Ring Combine(Ring ring)
	{
		...
		combinedRing.combinedRings.Add(base.getOne() as Ring);
