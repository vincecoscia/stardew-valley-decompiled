...
public class Hat : Item
{
	...
	public void load(string which)
	{
		Dictionary<string, string> hatInfo = DataLoader.Hats(Game1.content);
		if (!hatInfo.TryGetValue(which, out var rawData))
		{
			which = "0";
			rawData = hatInfo[which];
		}
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth, StackDrawType drawStackNumber, Color color, bool drawShadow)
	{
		...
		spriteBatch.Draw(texture, location + new Vector2(32f, 32f), drawnSourceRect, this.isPrismatic ? (Utility.GetPrismaticColor() * transparency) : (color * transparency), 0f, new Vector2(10f, 10f), 4f * scaleSize, SpriteEffects.None, layerDepth);
	...
	public void draw(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth, int direction, bool useAnimalTexture = false)
	{
		...
		spriteBatch.Draw(texture, location + new Vector2(10f, 10f), drawnSourceRect, this.isPrismatic ? (Utility.GetPrismaticColor() * transparency) : (Color.White * transparency), 0f, new Vector2(3f, 3f), 3f * scaleSize, SpriteEffects.None, layerDepth);
	}
	...
	{
		ParsedItemData itemData = ItemRegistry.GetDataOrErrorItem(base.QualifiedItemId);
		if (this.Name != null && itemData.IsErrorItem)
		{
