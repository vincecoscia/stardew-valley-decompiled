...
public class FairyBoxTrinketEffect : TrinketEffect
{
	...
	{
		this.damageTakenOrReceivedSinceLastHeal += damageAmount;
	...
	public override void Update(Farmer farmer, GameTime time, GameLocation location)
	{
		...
		{
			if (farmer.health < farmer.maxHealth && this.damageTakenOrReceivedSinceLastHeal >= 0)
			{
				int healAmount = (int)Math.Min(Math.Pow(this.damageTakenOrReceivedSinceLastHeal, 0.33000001311302185), (float)farmer.maxHealth / 10f);
				...
				if (healAmount > 0)
				{
					...
					this.damageTakenOrReceivedSinceLastHeal = 0;
				}
			}
			this.healTimer = 0f;
		}
	...
	public override void Apply(Farmer farmer)
	{
		this.healTimer = 0f;
		this.damageTakenOrReceivedSinceLastHeal = 0;
		base._companion = new FlyingCompanion(0);
		if (Game1.gameMode == 3)
		{
