...
public class TrainCar : INetObject<NetFields>
{
	...
	{
		b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, globalPosition), new Rectangle(192 + (int)this.carType * 128, 512 - (this.alternateCar ? 64 : 0), 128, 57), this.color.Value, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 256f) / 10000f);
		b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, globalPosition + new Vector2(0f, 228f)), new Rectangle(192 + (int)this.carType * 128, 569, 128, 7), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 256f) / 10000f);
		if ((int)this.carType == 1)
		{
			b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, globalPosition), new Rectangle(448 + (int)this.resourceType * 128 % 256, 576 + (int)this.resourceType / 2 * 32, 128, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 260f) / 10000f);
			if ((int)this.loaded > 0 && Game1.random.NextDouble() < 0.02 && globalPosition.X > 320f && globalPosition.X < (float)(location.map.DisplayWidth - 256))
			{
				this.loaded.Value--;
		...
		if ((int)this.frontDecal != -1 && ((int)this.carType == 0 || (int)this.carType == 1))
		{
			if ((int)this.frontDecal < 35)
			{
				b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, globalPosition + new Vector2(192f, 92f)), new Rectangle(224 + (int)this.frontDecal * 32 % 224, 576 + (int)this.frontDecal * 32 / 224 * 32, 32, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 260f) / 10000f);
			}
			else if ((int)this.frontDecal == 35)
			{
