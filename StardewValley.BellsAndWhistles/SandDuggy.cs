...
public class SandDuggy : INetObject<NetFields>
{
	...
	public virtual int FindRandomFreePoint()
	{
		...
		for (int j = 0; j < this.holeLocations.Count; j++)
		{
			Point hole_location2 = this.holeLocations[j];
			if (!this.locationRef.Value.isObjectAtTile(hole_location2.X, hole_location2.Y) && !this.locationRef.Value.isTerrainFeatureAt(hole_location2.X, hole_location2.Y) && !this.locationRef.Value.terrainFeatures.ContainsKey(Utility.PointToVector2(hole_location2)))
			{
		...
		for (int i = 0; i < valid_hole_locations.Count; i++)
		{
			...
			Point hole_location = this.holeLocations[index];
			bool near_farmer = false;
			foreach (Farmer farmer in this.locationRef.Value.farmers)
			{
				if (this.NearFarmer(hole_location, farmer))
				{
					near_farmer = true;
					break;
				}
	...
	public virtual void Update(GameTime time)
	{
		...
		if (this.currentHoleIndex.Value >= 0)
		{
			...
			if (this.NearFarmer(synched_position, Game1.player) && this.FindRandomFreePoint() != (int)this.currentHoleIndex)
			{
		...
		if ((int)this.currentHoleIndex == -1 || this.currentHoleIndex.Value != this._localIndex)
		{
