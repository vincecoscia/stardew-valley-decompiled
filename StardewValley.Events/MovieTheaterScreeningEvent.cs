...
public class MovieTheaterScreeningEvent
{
	...
	public Event getMovieEvent(string movieId, List<List<Character>> player_and_guest_audience_groups, List<List<Character>> npcOnlyAudienceGroups, Dictionary<Character, MovieConcession> concessions_data = null)
	{
		...
		if (this.playerAndGuestAudienceGroups.Count > 0)
		{
			for (int i13 = 0; i13 < 8; i13++)
			{
				...
				if (this.playerAndGuestAudienceGroups[whichGroup].Count == 2 && (seat13 == 3 || seat13 == 7))
				{
					i13++;
					seat13++;
					seat13 %= 8;
				}
				for (int j7 = 0; j7 < this.playerAndGuestAudienceGroups[whichGroup].Count && seat13 + j7 < backRow.Length; j7++)
				{
					backRow[seat13 + j7] = ((this.playerAndGuestAudienceGroups[whichGroup][j7] is Farmer) ? ("farmer" + Utility.getFarmerNumberFromFarmer(this.playerAndGuestAudienceGroups[whichGroup][j7] as Farmer)) : ((string)this.playerAndGuestAudienceGroups[whichGroup][j7].name));
					if (j7 > 0)
					{
						i13++;
					}
		...
		if (whichGroup < this.playerAndGuestAudienceGroups.Count)
		{
			...
			for (int i11 = 0; i11 < 4; i11++)
			{
				...
				for (int j6 = 0; j6 < this.playerAndGuestAudienceGroups[whichGroup].Count && seat12 + j6 < frontRow.Length; j6++)
				{
					frontRow[seat12 + j6] = ((this.playerAndGuestAudienceGroups[whichGroup][j6] is Farmer) ? ("farmer" + Utility.getFarmerNumberFromFarmer(this.playerAndGuestAudienceGroups[whichGroup][j6] as Farmer)) : ((string)this.playerAndGuestAudienceGroups[whichGroup][j6].name));
					if (j6 > 0)
					{
						i11++;
					}
			...
			if (whichGroup < this.playerAndGuestAudienceGroups.Count)
			{
				...
				for (int i10 = 0; i10 < 6; i10++)
				{
					...
					if (this.playerAndGuestAudienceGroups[whichGroup].Count == 2 && seat11 == 2)
					{
						i10++;
						seat11++;
						seat11 %= 8;
					}
					for (int j5 = 0; j5 < this.playerAndGuestAudienceGroups[whichGroup].Count && seat11 + j5 < midRow.Length; j5++)
					{
						midRow[seat11 + j5] = ((this.playerAndGuestAudienceGroups[whichGroup][j5] is Farmer) ? ("farmer" + Utility.getFarmerNumberFromFarmer(this.playerAndGuestAudienceGroups[whichGroup][j5] as Farmer)) : ((string)this.playerAndGuestAudienceGroups[whichGroup][j5].name));
						if (j5 > 0)
						{
							i10++;
						}
		...
		for (int i8 = 0; i8 < backRow.Length; i8++)
		{
			...
			Point seat9 = this.getBackRowSeatTileFromIndex(i8);
		...
		for (int i7 = 0; i7 < midRow.Length; i7++)
		{
			...
			sittingTogetherCount++;
			Point seat8 = this.getMidRowSeatTileFromIndex(i7);
		...
		for (int i6 = 0; i6 < frontRow.Length; i6++)
		{
			...
			sittingTogetherCount++;
			Point seat7 = this.getFrontRowSeatTileFromIndex(i6);
		...
		for (int i5 = 0; i5 < backRow.Length; i5++)
		{
			...
			{
				Point seat6 = this.getBackRowSeatTileFromIndex(i5);
				if (soFar == 1)
				{
		...
		for (int i4 = 0; i4 < midRow.Length; i4++)
		{
			...
			{
				Point seat5 = this.getMidRowSeatTileFromIndex(i4);
				if (soFar == 1)
				{
		...
		for (int i3 = 0; i3 < frontRow.Length; i3++)
		{
			...
			{
				Point seat4 = this.getFrontRowSeatTileFromIndex(i3);
				if (soFar == 1)
				{
		...
		for (int i2 = 0; i2 < backRow.Length; i2++)
		{
			...
			{
				Point seat3 = this.getBackRowSeatTileFromIndex(i2);
		...
		for (int n = 0; n < midRow.Length; n++)
		{
			...
			{
				Point seat2 = this.getMidRowSeatTileFromIndex(n);
		...
		for (int m = 0; m < frontRow.Length; m++)
		{
			...
			{
				Point seat = this.getFrontRowSeatTileFromIndex(m);
		...
		{
			string actorName = MovieTheaterScreeningEvent.getEventName(c);
		...
		foreach (MovieScene scene2 in this.movieData.Scenes)
		{
					...
					if (!this._whiteListDependencyLookup.ContainsKey(responding_characters[k]))
					{
						...
						if (movie_reaction2.Whitelist != null)
						{
							for (int j3 = 0; j3 < movie_reaction2.Whitelist.Count; j3++)
							{
								...
								this._whiteListDependencyLookup[white_list_character2] = responding_characters[k];
			...
			if (!found_reaction)
			{
				for (int j = 0; j < responding_characters.Count; j++)
				{
					...
					{
						if (!movie_reaction.ShouldApplyToMovie(this.movieData, MovieTheater.GetPatronNames(), MovieTheater.GetResponseForMovie(responding_characters[j] as NPC)) || movie_reaction.SpecialResponses?.DuringMovie == null || !(movie_reaction.SpecialResponses.DuringMovie.ResponsePoint == current_response_index.ToString()))
						{
							continue;
						}
						if (!this._whiteListDependencyLookup.ContainsKey(responding_characters[j]))
						{
							this._responseOrder[current_response_index] = responding_characters[j];
							if (movie_reaction.Whitelist != null)
							{
								for (int j2 = 0; j2 < movie_reaction.Whitelist.Count; j2++)
								{
									...
									this._whiteListDependencyLookup[white_list_character] = responding_characters[j];
		...
		{
			if (!this._whiteListDependencyLookup.ContainsKey(responding_characters[i]))
			{
				for (; this._responseOrder.ContainsKey(current_response_index); current_response_index++)
				{
				}
				this._responseOrder[current_response_index] = responding_characters[i];
				current_response_index++;
			}
		}
	...
	protected void _ParseScene(StringBuilder sb, MovieScene scene)
	{
		...
		if (scene.Image >= 0)
		{
			sb.Append("/specificTemporarySprite movieTheater_screen " + this.movieData.Id + " " + scene.Image + " " + scene.Shake);
			if (this.movieData.Texture != null)
			{
				...
				handler.AppendFormatted(ArgUtility.EscapeQuotes(this.movieData.Texture));
				handler.AppendLiteral("\"");
