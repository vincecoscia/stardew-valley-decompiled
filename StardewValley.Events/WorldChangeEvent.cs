...
public class WorldChangeEvent : BaseFarmEvent
{
	...
	public void resetForPlayerEntry(Point targetTile)
	{
		...
		case 15L:
		{
			...
			for (int l = 0; l < bags; l++)
			{
				this.location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(184, 104, 14, 15), new Vector2(39 + l % 3, 34.1f + (float)(l / 3) * 0.5f) * 64f, flipped: false, 0f, Color.White)
				{
					animationLength = 1,
					interval = 700f,
					totalNumberOfLoops = 999,
					layerDepth = 0.1f + (float)l * 0.01f,
					scale = 4f
				});
			}
			...
			for (int k = 0; k < 20; k++)
			{
				...
				TemporaryAnimatedSprite parrot2 = new TemporaryAnimatedSprite("LooseSprites\\parrots", new Rectangle(48 + Game1.random.Next(2) * 72, 96, 24, 24), start_point2, flipped: false, 0f, Color.White)
				{
					...
					id = 778,
					scale = 4f,
					yStopCoordinate = (int)start_point2.Y + Game1.random.Next(19, 27) * 64 + k * 64 / 2,
				...
				DelayedAction.playSoundAfterDelay("parrot_flap", 500 + k * 250);
				DelayedAction.playSoundAfterDelay("parrot_flap", 5500 + k * 250);
	...
	public override void makeChangesToLocation()
	{
		...
		if ((int)this.whichEvent == 15 && Game1.IsMasterGame)
		{
