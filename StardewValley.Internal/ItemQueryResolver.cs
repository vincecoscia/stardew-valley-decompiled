...
public static class ItemQueryResolver
{
	...
	public static class DefaultResolvers
	{
			...
			foreach (IItemDataDefinition itemDataDefinition in ItemRegistry.ItemTypes)
			{
				...
				if (typeId == "(F)")
				{
					...
					foreach (ParsedItemData data in itemDataDefinition.GetAllData())
					{
						...
						{
							Furniture item2 = ItemRegistry.Create<Furniture>(data.QualifiedItemId);
							if (!requirePrice || item2.salePrice(ignoreProfitMargins: true) > 0)
							{
								furniture.Add(item2);
							}
						}
		...
		public static IEnumerable<ItemQueryResult> LOST_BOOK_OR_ITEM(string key, string arguments, ItemQueryContext context, bool avoidRepeat, HashSet<string> avoidItemIds, Action<string, string> logError)
		{
			...
			return ItemQueryResolver.TryResolve(arguments, new ItemQueryContext(context));
		}
		...
		public static IEnumerable<ItemQueryResult> MONSTER_SLAYER_REWARDS(string key, string arguments, ItemQueryContext context, bool avoidRepeat, HashSet<string> avoidItemIds, Action<string, string> logError)
		{
			...
			for (int m = 0; m < array.Length; m++)
			{
				...
				string id4 = pair3.Key;
				MonsterSlayerQuestData questData4 = pair3.Value;
				if (!questIds.Contains(id4) && questData4.RewardItemId != null && questData4.RewardItemPrice != -1 && ItemContextTagManager.HasBaseTag(questData4.RewardItemId, "item_type_ring"))
				{
			...
			for (int m = 0; m < array.Length; m++)
			{
				...
				string id4 = pair4.Key;
				MonsterSlayerQuestData questData3 = pair4.Value;
				if (!questIds.Contains(id4) && questData3.RewardItemId != null && questData3.RewardItemPrice != -1 && !(ItemRegistry.ResolveMetadata(questData3.RewardItemId)?.GetTypeDefinition()?.Identifier != "(H)"))
				{
			...
			for (int m = 0; m < array.Length; m++)
			{
				...
				string id4 = pair2.Key;
				MonsterSlayerQuestData questData2 = pair2.Value;
				if (!questIds.Contains(id4) && questData2.RewardItemId != null && questData2.RewardItemPrice != -1 && !(ItemRegistry.ResolveMetadata(questData2.RewardItemId)?.GetTypeDefinition()?.Identifier != "(W)"))
				{
			...
			for (int m = 0; m < array.Length; m++)
			{
				...
				if (!questIds.Contains(id4) && questData.RewardItemId != null && questData.RewardItemPrice != -1)
				{
	...
	{
		return ItemQueryResolver.ApplyItemFields(item, data.MinStack, data.MaxStack, data.ToolUpgradeLevel, data.ObjectInternalName, data.ObjectDisplayName, data.Quality, data.IsRecipe, data.StackModifiers, data.StackModifierMode, data.QualityModifiers, data.QualityModifierMode, data.ModData, context, inputItem);
	}
