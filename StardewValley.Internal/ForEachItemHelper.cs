...
public static class ForEachItemHelper
{
	...
	public static bool ForEachItemInWorld(ForEachItemDelegate handler)
	{
		...
		foreach (Farmer allFarmer in Game1.getAllFarmers())
		{
			...
			if (!ForEachItemHelper.ApplyToList(farmer.Items, handler, leaveNullSlotsOnRemoval: true, OnChangedItemSlot) || !ForEachItemHelper.ApplyToField(farmer.shirtItem, handler, OnChangedEquipment) || !ForEachItemHelper.ApplyToField(farmer.pantsItem, handler, OnChangedEquipment) || !ForEachItemHelper.ApplyToField(farmer.boots, handler, OnChangedEquipment) || !ForEachItemHelper.ApplyToField(farmer.hat, handler, OnChangedEquipment) || !ForEachItemHelper.ApplyToField(farmer.leftRing, handler, OnChangedEquipment) || !ForEachItemHelper.ApplyToField(farmer.rightRing, handler, OnChangedEquipment) || !ForEachItemHelper.ApplyToItem(farmer.recoveredItem, handler, delegate
			{
			...
			}) || !ForEachItemHelper.ApplyToField(farmer.toolBeingUpgraded, handler) || !ForEachItemHelper.ApplyToList(farmer.itemsLostLastDeath, handler))
			{
				return false;
			}
		...
		if (!ForEachItemHelper.ApplyToList(Game1.player.team.returnedDonations, handler))
		{
			return false;
		}
		foreach (Inventory value in Game1.player.team.globalInventories.Values)
		{
			if (!ForEachItemHelper.ApplyToList(value, handler))
			{
				return false;
			}
		}
		...
		{
			if (!ForEachItemHelper.ApplyToList(specialOrder.donatedItems, handler))
			{
				return false;
			}
		}
	...
	public static bool ForEachItemInLocation(GameLocation location, ForEachItemDelegate handler)
	{
		...
		if (!ForEachItemHelper.ApplyToList(location.furniture, handler))
		{
			return false;
		}
		foreach (NPC character in location.characters)
		{
			if (!(character is Child child))
			{
				...
				{
					if (character is Pet pet && !ForEachItemHelper.ApplyToField(pet.hat, handler))
					{
						return false;
					}
				}
				else if (!ForEachItemHelper.ApplyToField(horse.hat, handler))
				{
					return false;
				}
			}
			else if (!ForEachItemHelper.ApplyToField(child.hat, handler))
			{
				return false;
			}
		}
		...
		if (location.objects.Length > 0)
		{
			...
			{
				if (!ForEachItemHelper.ApplyToItem(location.objects[tile], handler, delegate
				{
					location.objects.Remove(tile);
				}, delegate(Item newItem)
				{
					location.objects[tile] = ForEachItemHelper.PrepareForReplaceWith(location.objects[tile], (Object)newItem);
				}))
				{
					return false;
				}
		...
		for (int i = location.debris.Count - 1; i >= 0; i--)
		{
			...
			if (d.item != null && !ForEachItemHelper.ApplyToItem(d.item, handler, Remove, ReplaceWith))
			{
				return false;
			}
			void Remove()
		...
		{
			if (!ForEachItemHelper.ApplyToList(shopLocation.itemsFromPlayerToSell, handler))
			{
				return false;
			}
			if (!ForEachItemHelper.ApplyToList(shopLocation.itemsToStartSellingTomorrow, handler))
			{
				return false;
			}
		}
	...
	public static bool ApplyToField<TItem>(NetRef<TItem> field, ForEachItemDelegate handler, Action<Item, Item> onChanged = null) where TItem : Item
	{
		...
		return ForEachItemHelper.ApplyToItem(field.Value, handler, delegate
		{
			field.Value = null;
	...
	public static bool ApplyToList<TItem>(IList<TItem> list, ForEachItemDelegate handler, bool leaveNullSlotsOnRemoval = false, Action<Item, Item, int> onChanged = null) where TItem : Item
	{
		for (int i = list.Count - 1; i >= 0; i--)
		{
			...
			if (!ForEachItemHelper.ApplyToItem(list[i], handler, Remove, ReplaceWith))
			{
				return false;
			}
			void Remove()
