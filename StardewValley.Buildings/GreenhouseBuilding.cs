...
public class GreenhouseBuilding : Building
{
	...
	public override bool doesTileHaveProperty(int tile_x, int tile_y, string property_name, string layer_name, ref string property_value)
	{
		...
		if (layer_name == "Back" && ((tile_x >= (int)base.tileX - 1 && tile_x <= (int)base.tileX + (int)base.tilesWide - 1 && tile_y <= (int)base.tileY + (int)base.tilesHigh && tile_y >= (int)base.tileY) || (this.CanDrawEntranceTiles() && tile_x >= (int)base.tileX + 1 && tile_x <= (int)base.tileX + (int)base.tilesWide - 2 && tile_y == (int)base.tileY + (int)base.tilesHigh + 1)))
		{
			if (this.CanDrawEntranceTiles() && tile_x >= (int)base.tileX + base.humanDoor.X - 1 && tile_x <= (int)base.tileX + base.humanDoor.X + 1 && tile_y <= (int)base.tileY + (int)base.tilesHigh + 1 && tile_y >= (int)base.tileY + base.humanDoor.Y + 1)
			{
	...
	public virtual void DrawEntranceTiles(SpriteBatch b)
	{
		...
		if (tilesheet != null)
		{
			...
			{
				float draw_layer = 0f;
				Vector2 vector_draw_position = Game1.GlobalToLocal(Game1.viewport, new Vector2((int)base.tileX + base.humanDoor.Value.X - 1, (int)base.tileY + base.humanDoor.Value.Y + 1) * 64f);
				...
				vector_draw_position = Game1.GlobalToLocal(Game1.viewport, new Vector2((int)base.tileX + base.humanDoor.Value.X - 1, (int)base.tileY + base.humanDoor.Value.Y + 2) * 64f);
	...
	public override void drawShadow(SpriteBatch b, int localX = -1, int localY = -1)
	{
		...
		b.Draw(base.texture.Value, Game1.GlobalToLocal(Game1.viewport, new Vector2(((int)base.tileX - 1) * 64, (int)base.tileY * 64)), shadow_rectangle, Color.White * ((localX == -1) ? base.alpha : 1f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 0f);
	}
}
