...
public class ResourceClump : TerrainFeature
{
	...
	public override bool performToolAction(Tool t, int damage, Vector2 tileLocation)
	{
		...
		if (t is Axe && t.hasEnchantmentOfType<ShavingEnchantment>() && Game1.random.NextDouble() <= (double)(power / 12f) && ((int)this.parentSheetIndex == 602 || (int)this.parentSheetIndex == 600))
		{
		...
		Game1.createRadialDebris(Game1.currentLocation, radialDebris, (int)tileLocation.X + Game1.random.Next((int)this.width / 2 + 1), (int)tileLocation.Y + Game1.random.Next((int)this.height / 2 + 1), Game1.random.Next(4, 9), resource: false);
	...
	public bool destroy(Tool t, GameLocation location, Vector2 tileLocation)
	{
		if (t != null && location.HasUnlockedAreaSecretNotes(t.getLastFarmerToUse()) && Game1.random.NextDouble() < 0.05)
		{
			...
			if (o != null)
			{
				Game1.createItemDebris(o, tileLocation * 64f + new Vector2((int)this.width / 2, (int)this.height / 2) * 64f, -1, location);
			}
		}
		switch (this.parentSheetIndex)
		{
		...
		case 602L:
		{
			...
			int numChunks = (((int)this.parentSheetIndex == 602) ? 8 : 2);
			Random r;
			...
			if (Game1.IsMultiplayer)
			{
				Game1.createMultipleItemDebris(hardwood, tileLocation * 64f + new Vector2((float)(int)this.width / 4f, (float)(int)this.height / 4f) * 64f, -1, Game1.currentLocation);
			}
			else
			{
				Game1.createMultipleItemDebris(hardwood, tileLocation * 64f + new Vector2((float)(int)this.width / 4f, (float)(int)this.height / 4f) * 64f, -1, Game1.currentLocation);
		...
		case 758L:
		{
			...
			int numChunks2 = (((int)this.parentSheetIndex == 672) ? 15 : 10);
			if (Game1.IsMultiplayer)
			{
				Game1.recentMultiplayerRandom = Utility.CreateRandom((double)tileLocation.X * 1000.0, tileLocation.Y);
			...
			{
				Game1.createRadialDebris(Game1.currentLocation, 390, (int)tileLocation.X, (int)tileLocation.Y, numChunks2, resource: false, -1, item: true);
			}
		...
		case 622L:
			...
			if (Game1.IsMultiplayer)
			{
				...
				{
					Game1.createMultipleItemDebris(ItemRegistry.Create("(O)74"), tileLocation * 64f + new Vector2((float)(int)this.width / 4f, (float)(int)this.height / 4f) * 64f, -1, Game1.currentLocation);
				}
			}
			else
			{
				Game1.createMultipleItemDebris(ItemRegistry.Create("(O)386", 10), tileLocation * 64f + new Vector2((float)(int)this.width / 4f, (float)(int)this.height / 4f) * 64f, -1, Game1.currentLocation);
				Game1.createMultipleItemDebris(ItemRegistry.Create("(O)390", 8), tileLocation * 64f + new Vector2((float)(int)this.width / 4f, (float)(int)this.height / 4f) * 64f, -1, Game1.currentLocation);
		...
		case 46L:
		{
			...
			Game1.createMultipleItemDebris(ItemRegistry.Create("(O)Moss", ran.Next(2, 4)), tileLocation * 64f + new Vector2((float)(int)this.width / 4f, (float)(int)this.height / 4f) * 64f, -1, Game1.currentLocation);
			Game1.createMultipleItemDebris(ItemRegistry.Create("(O)771", ran.Next(2, 4)), tileLocation * 64f + new Vector2((float)(int)this.width / 3f, (float)(int)this.height / 3f) * 64f, -1, Game1.currentLocation);
			if (ran.NextDouble() < 0.05)
			{
				Game1.createMultipleItemDebris(ItemRegistry.Create("(O)MossySeed"), tileLocation * 64f + new Vector2((float)(int)this.width / 4f, (float)(int)this.height / 4f) * 64f, -1, Game1.currentLocation);
			}
			return true;
		}
		default:
	...
	public override Rectangle getBoundingBox()
	{
		...
		return new Rectangle((int)tileLocation.X * 64, (int)tileLocation.Y * 64, (int)this.width * 64, (int)this.height * 64);
	}

	public bool occupiesTile(int x, int y)
	{
		Vector2 tile = this.Tile;
		if ((float)x >= tile.X && (float)x - tile.X < (float)(int)this.width && (float)y >= tile.Y)
		{
			return (float)y - tile.Y < (float)(int)this.height;
		}
		return false;
	}
	...
	public override void draw(SpriteBatch spriteBatch)
	{
		...
		Rectangle sourceRect = Game1.getSourceRectForStandardTileSheet(this.texture, this.parentSheetIndex, 16, 16);
		sourceRect.Width = (int)this.width * 16;
		sourceRect.Height = (int)this.height * 16;
		Vector2 tile = this.Tile;
