...
public class GiantCrop : ResourceClump
{
	...
	public override bool performToolAction(Tool t, int damage, Vector2 tileLocation)
	{
		...
		Game1.createRadialDebris(Game1.currentLocation, 12, (int)tileLocation.X + (int)base.width / 2, (int)tileLocation.Y + (int)base.height / 2, r.Next(4, 9), resource: false);
		...
		if (t.hasEnchantmentOfType<ShavingEnchantment>() && r.NextBool((float)power / 5f) && data?.HarvestItems != null)
		{
			...
			{
				Item item2 = this.TryGetDrop(drop2, r, player, isShaving: true, healthDeducted);
				if (item2 != null)
				{
					...
					Debris d = new Debris(item2, new Vector2((tileLocation.X + (float)((int)base.width / 2)) * 64f, (tileLocation.Y + (float)((int)base.height / 2)) * 64f), Game1.player.getStandingPosition());
		...
		if (base.health.Value <= 0f)
		{
			...
			if (data?.HarvestItems != null)
			{
				...
				{
					Item item = this.TryGetDrop(drop, r, player, isShaving: false, healthDeducted);
					if (item == null)
					{
						continue;
					}
					if (this.Id.Equals("QiFruit") && !Game1.player.team.SpecialOrderActive("QiChallenge2"))
					{
						...
						Game1.createMultipleItemDebris(ItemRegistry.Create("(O)MysteryBox"), new Vector2((int)tileLocation.X + (int)base.width / 2, (int)tileLocation.Y + (int)base.width / 2) * 64f, -2, location);
					}
					else
					{
						Game1.createMultipleItemDebris(item, new Vector2((int)tileLocation.X + (int)base.width / 2, (int)tileLocation.Y + (int)base.width / 2) * 64f, -2, location);
						Game1.setRichPresence("giantcrop", item.Name);
					}
				}
			}
			Game1.createRadialDebris(Game1.currentLocation, 12, (int)tileLocation.X + (int)base.width / 2, (int)tileLocation.Y + (int)base.width / 2, r.Next(4, 9), resource: false);
			...
			for (int x = 0; x < (int)base.width; x++)
			{
				for (int y = 0; y < (int)base.height; y++)
				{
					...
					if ((int)base.width >= 2 && (int)base.height >= 2 && (x == 0 || x == (int)base.width - 2) && (y == 0 || y == (int)base.height - 2))
					{
	...
	private Item TryGetDrop(GiantCropHarvestItemData drop, Random r, Farmer targetFarmer, bool isShaving, float healthDeducted)
	{
		...
		Item item = ItemQueryResolver.TryResolveRandomItem(drop, new ItemQueryContext(this.Location, targetFarmer, r), avoidRepeat: false, null, null, null, delegate(string query, string error)
		{
