...
public class Flooring : TerrainFeature
{
	...
	{
		if (this.Location != null && !this.Location.isGreenhouse && this.GetData().WinterTexture != null)
		{
	...
	public override void draw(SpriteBatch spriteBatch)
	{
		...
		byte drawSum = (byte)(this.neighborMask & 0xFu);
	...
	private List<Neighbor> gatherNeighbors()
	{
		...
		for (int k = 0; k < offsets.Length; k++)
		{
			...
			{
				Neighbor i = new Neighbor(null, item.Direction, item.InvDirection);
				results.Add(i);
			}
			else if (terrainFeatures.TryGetValue(tile, out feature) && feature is Flooring flooring && flooring.whichFloor == this.whichFloor)
			{
				Neighbor j = new Neighbor(flooring, item.Direction, item.InvDirection);
				results.Add(j);
			}
		}
		return results;
