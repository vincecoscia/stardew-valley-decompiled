...
public class Tree : TerrainFeature
{
	...
	public override bool tickUpdate(GameTime time)
	{
		...
		if (!this.falling)
		{
			...
			if (this.maxShake > 0f)
			{
				this.maxShake = Math.Max(0f, this.maxShake - (((int)this.growthStage >= 5) ? 0.0010226539f : 0.0030679617f));
			}
		}
		else
		{
			...
			if ((double)Math.Abs(this.shakeRotation) > Math.PI / 2.0)
			{
				...
				if (data != null)
				{
					...
					{
						int numToDrop = (int)((Game1.getFarmer(this.lastPlayerToHit.Value).professions.Contains(12) ? 1.25 : 1.0) * (double)(12 + this.extraWoodCalculator(tileLocation)));
						if (Game1.getFarmer(this.lastPlayerToHit.Value).stats.Get("Book_Woodcutting") != 0 && r.NextDouble() < 0.05)
						{
							numToDrop *= 2;
						}
						Game1.createRadialDebris(location, 12, (int)tileLocation.X + (this.shakeLeft ? (-4) : 4), (int)tileLocation.Y, numToDrop, resource: true);
			...
			if ((int)this.growthStage >= 5)
			{
				...
				if ((bool)this.hasSeed && (Game1.IsMultiplayer || Game1.player.ForagingLevel >= 1))
				{
					...
					if (data != null && data.SeedDropItems?.Count > 0)
					{
						...
						{
							Item seed2 = this.TryGetDrop(drop, Game1.random, Game1.player, "SeedDropItems");
							if (seed2 != null)
							{
	...
	public void onGreenRainDay(bool undo = false)
	{
		if (undo)
		{
			if ((bool)this.isTemporaryGreenRainTree)
			{
		...
		else
		{
			...
			if ((int)this.growthStage < 5)
			{
				if ((int)this.growthStage == 0 && (Game1.random.NextDouble() < 0.5 || this.Location == null || this.Location.objects.ContainsKey(this.Tile)))
				{
					return;
				}
			...
			if ((this.treeType == "1" || this.treeType == "2") && (int)this.growthStage >= 5 && Game1.random.NextBool(0.75))
			{
	...
	public override void dayUpdate()
	{
		...
		if ((int)this.growthStage >= 5 && !this.stump.Value && environment is Farm && Game1.random.NextBool(data?.SeedSpreadChance ?? 0.15f))
		{
		...
		if ((bool)this.isTemporaryGreenRainTree && environment.IsGreenhouse && (this.localSeason == Season.Winter || this.localSeason == Season.Fall))
		{
			this.hasSeed.Value = false;
		}
		else
		{
			this.hasSeed.Value = data != null && data.SeedItemId != null && (int)this.growthStage >= 5 && Game1.random.NextBool(data.SeedOnShakeChance);
		}
		...
		if (flag.HasValue && flag.GetValueOrDefault() && (int)this.growthStage >= 14 && !this.stump.Value && Game1.random.NextBool(mossChance))
		{
			this.hasMoss.Value = true;
		}
	}
	...
	{
		if (Game1.IsFall && Game1.random.NextDouble() < 0.05 && !this.tapped && (this.treeType.Value == "1" || this.treeType.Value == "2") && this.growthStage.Value >= 5 && this.Location != null && !(this.Location is Town) && !this.Location.IsGreenhouse)
		{
			if (this.treeType == "1")
			{
				this.treeType.Value = "10";
		...
		{
			Object tile_object = this.Location.getObjectAtTile((int)this.Tile.X, (int)this.Tile.Y);
			if (tile_object != null && tile_object.IsTapper())
			{
				this.UpdateTapperProduct(tile_object, null, onlyPerformRemovals: true);
			}
		}
		this.loadSprite();
		return false;
	...
	public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation)
	{
		...
		if ((int)this.growthStage >= 5)
		{
			...
			if (t is Axe)
			{
				...
				if (location is Town && tileLocation.X < 100f && !this.isTemporaryGreenRainTree)
				{
				...
				else if (!this.stump && t.getLastFarmerToUse() != null && Utility.tryRollMysteryBox(0.005))
				{
				...
				else if (!this.stump && t.getLastFarmerToUse() != null && t.getLastFarmerToUse().stats.Get("TreesChopped") > 20 && Game1.random.NextDouble() < 0.0003 + (t.getLastFarmerToUse().mailReceived.Contains("GotWoodcuttingBook") ? 0.0007 : ((double)t.getLastFarmerToUse().stats.Get("TreesChopped") * 1E-05)))
				{
		...
		else if ((int)this.growthStage >= 3)
		{
			...
			this.shake(tileLocation, doEvenIfStillShaking: true);
			float damage2 = 1f;
			damage2 = ((explosion > 0) ? ((float)explosion) : (t.upgradeLevel switch
			{
				0L => 2f, 
				1L => 2.5f, 
				2L => 3.34f, 
			...
			if (data != null)
			{
				...
				else if (Game1.IsMultiplayer)
				{
					...
					if (chopItems != null && chopItems.Count > 0)
					{
						...
						foreach (WildTreeChopItemData drop in data.ChopItems)
						{
							Item item = this.TryGetDrop(drop, r, targetFarmer2, "ChopItems");
							if (item != null)
							{
				...
				else
				{
					...
					if (chopItems2 != null && chopItems2.Count > 0)
					{
						...
						{
							Item item2 = this.TryGetDrop(drop2, r, targetFarmer, "ChopItems");
							if (item2 != null)
							{
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 positionOnScreen, Vector2 tileLocation, float scale, float layerDepth)
	{
		...
		if ((int)this.growthStage < 5)
		{
			Microsoft.Xna.Framework.Rectangle sourceRect = this.growthStage switch
			{
	...
	public override void draw(SpriteBatch spriteBatch)
	{
		...
		else
		{
			...
			{
				spriteBatch.Draw(this.texture.Value, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f + ((this.shakeTimer > 0f) ? ((float)Math.Sin(Math.PI * 2.0 / (double)this.shakeTimer) * 3f) : 0f), tileLocation.Y * 64f - 64f)), stumpSource, Color.White * this.alpha, 0f, Vector2.Zero, 4f, this.flipped ? SpriteEffects.FlipHorizontally : SpriteEffects.None, baseSortPosition / 10000f);
			}
