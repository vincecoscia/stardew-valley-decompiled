...
public class GalaxySocket
{
	...
	public void Receive(Action<GalaxyID> onConnection, Action<GalaxyID, Stream> onMessage, Action<GalaxyID> onDisconnect, Action<string> onError)
	{
		...
		try
		{
			...
			if (lobbyVersion != "" && lobbyVersion != this.protocolVersion)
			{
		...
		this.ghosts.IntersectWith(from peer in this.LobbyMembers()
		...
		while (GalaxyInstance.Networking().ReadP2PPacket(buffer, (uint)buffer.Length, ref packetSize, ref sender))
		{
			...
			if (this.incompletePackets.ContainsKey(sender.ToUint64()))
			{
				messageData.Position = 0L;
				messageData.CopyTo(this.incompletePackets[sender.ToUint64()]);
				if (!incomplete)
				{
					messageData = this.incompletePackets[sender.ToUint64()];
	...
	public virtual void DisconnectPeers(Action<GalaxyID> onDisconnect)
	{
		...
		foreach (GalaxyID peer2 in this.connections.Values)
		{
			if (this.lobby == null || !this.lobbyContains(peer2) || this.ghosts.Contains(peer2.ToUint64()))
			{
