...
public class GameServer : IGameServer, IBandwidthMonitor
{
	...
	public void receiveMessages()
	{
		...
		foreach (Action action2 in this.pendingGameAvailableActions.Keys)
		{
			if (this.pendingGameAvailableActions[action2]())
			{
				action2();
				this.completedPendingActions.Add(action2);
			}
		}
		foreach (Action action in this.completedPendingActions)
		{
			this.pendingGameAvailableActions.Remove(action);
		}
		this.completedPendingActions.Clear();
		if (Game1.chatBox == null)
		{
	...
	{
		if (!Game1.options.enableFarmhandCreation && !this.IsLocalMultiplayerInitiatedServer() && !farmhand.isCustomized)
		{
			return false;
		}
	...
	public void sendAvailableFarmhands(string userId, string connectionId, Action<OutgoingMessage> sendMessage)
	{
		...
		{
			if ((!farmhand2.Value.isActive() || Game1.multiplayer.isDisconnecting(farmhand2.Value.UniqueMultiplayerID)) && this.IsFarmhandAvailable(farmhand2.Value))
			{
