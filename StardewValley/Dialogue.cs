...
public class Dialogue
{
			...
			if (curDialogue.StartsWith('$'))
			{
				...
				case "$t":
					this.dialogues.Add(new DialogueLine("", delegate
					{
						...
						{
							Game1.log.Warn($"Failed to parse {"$t"} token for {translationKey ?? this.speaker?.Name ?? ("\"" + masterString + "\"")}: {error2}.");
						}
						else
						{
				...
				case "$q":
				{
					...
					{
						if (this.farmer.DialogueQuestionsAnswered.Contains(answerIDs[l]))
						{
				...
				case "$p":
				{
					...
					{
						if (this.farmer.DialogueQuestionsAnswered.Contains(prerequisiteSplit[k]))
						{
							choseOne = true;
							break;
						}
				...
				case "$y":
				{
					...
					{
						this.playerResponses.Add(new NPCDialogueResponse(null, -1, "quickResponse" + j, Game1.parseText(rawSplit[j])));
						this.quickResponses.Add(rawSplit[j + 1].Replace("*", "#$b#"));
					}
					break;
				}
				default:
	...
	{
		if (this.dialogues.Count > 0 && this.speaker != null && (bool)this.speaker.shouldWearIslandAttire && Game1.player.friendshipData.TryGetValue(this.speaker.Name, out var friendship) && friendship.IsDivorced() && this.CurrentEmotion == "$u")
		{
	...
	public virtual void prepareCurrentDialogueForDisplay()
	{
		...
		if (flag.HasValue && flag.GetValueOrDefault() && !this.farmer.canUnderstandDwarves)
		{
	...
	public string ReplacePlayerEnteredStrings(string str)
	{
		...
		if (str.Contains('%'))
		{
			...
			string farmName = Utility.FilterUserName(this.farmer.farmName);
			str = str.Replace("%farm", farmName);
			string favoriteThing = Utility.FilterUserName(this.farmer.favoriteThing);
	...
	private void checkForSpecialDialogueAttributes()
	{
		...
		if (this.dialogues.Count > 0 && this.dialogues[this.currentDialogueIndex].Text.Contains("{"))
		{
			this.dialogues[this.currentDialogueIndex].Text = this.dialogues[this.currentDialogueIndex].Text.Replace("{", "");
		...
		if (this.dialogues[this.currentDialogueIndex].Text.Contains("%noturn"))
		{
			this.dialogues[this.currentDialogueIndex].Text = this.dialogues[this.currentDialogueIndex].Text.Replace("%noturn", "");
	...
	private void checkEmotions()
	{
		...
		string dialogue = this.dialogues[this.currentDialogueIndex].Text;
		...
		if (dialogue.Contains("$h"))
		{
			...
			this.dialogues[this.currentDialogueIndex].Text = dialogue.Replace("$h", "");
			return;
		}
		if (dialogue.Contains("$s"))
		{
			this.CurrentEmotion = "$s";
			this.dialogues[this.currentDialogueIndex].Text = dialogue.Replace("$s", "");
			return;
		}
		if (dialogue.Contains("$u"))
		{
			this.CurrentEmotion = "$u";
			this.dialogues[this.currentDialogueIndex].Text = dialogue.Replace("$u", "");
			return;
		}
		if (dialogue.Contains("$l"))
		{
			this.CurrentEmotion = "$l";
			this.dialogues[this.currentDialogueIndex].Text = dialogue.Replace("$l", "");
			return;
		}
		if (dialogue.Contains("$a"))
		{
			this.CurrentEmotion = "$a";
			this.dialogues[this.currentDialogueIndex].Text = dialogue.Replace("$a", "");
			return;
		}
		int digits = 0;
		...
		if (digits > 0)
		{
			string emote = (this.CurrentEmotion = dialogue.Substring(emoteIndex, digits + 1));
			this.dialogues[this.currentDialogueIndex].Text = dialogue.Replace(emote, "");
		}
	}
