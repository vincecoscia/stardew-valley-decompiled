...
public class CraftingRecipe
{
	...
	public CraftingRecipe(string name, bool isCookingRecipe)
	{
		...
		{
			this.recipeList.Add(ingredients[j], ArgUtility.GetInt(ingredients, j + 1, 1));
		}
		string[] outputItems = ArgUtility.SplitBySpace(rawOutputItems);
		for (int i = 0; i < outputItems.Length; i += 2)
		{
			this.itemToProduce.Add(outputItems[i]);
			this.numberProducedPerCraft = ArgUtility.GetInt(outputItems, i + 1, 1);
		}
	...
	{
		ParsedItemData dataOrErrorItem = ItemRegistry.GetDataOrErrorItem(this.bigCraftable ? ("(BC)" + this.getIndexOfMenuView()) : this.getIndexOfMenuView());
	...
	public virtual ParsedItemData GetItemData(bool useFirst = false)
	{
		...
		return ItemRegistry.GetDataOrErrorItem(this.bigCraftable ? ("(BC)" + id) : ("(O)" + id));
	}
	...
	public virtual void consumeIngredients(List<IInventory> additionalMaterials)
	{
		foreach (KeyValuePair<string, int> pair in this.recipeList)
		{
			...
			{
				if (CraftingRecipe.ItemMatchesForCrafting(Game1.player.Items[j], itemId))
				{
			...
			for (int c = 0; c < additionalMaterials.Count; c++)
			{
				...
				{
					if (CraftingRecipe.ItemMatchesForCrafting(items[i], itemId))
					{
	...
	public static void ConsumeAdditionalIngredients(List<KeyValuePair<string, int>> additionalRecipeItems, List<IInventory> additionalMaterials)
	{
		for (int k = additionalRecipeItems.Count - 1; k >= 0; k--)
		{
			...
			for (int j = Game1.player.Items.Count - 1; j >= 0; j--)
			{
				if (CraftingRecipe.ItemMatchesForCrafting(Game1.player.Items[j], item_index))
				{
					int toRemove = required_count;
					required_count -= Game1.player.Items[j].Stack;
			...
			if (additionalMaterials != null && !foundInBackpack)
			{
				for (int c = 0; c < additionalMaterials.Count; c++)
				{
					...
					for (int i = items.Count - 1; i >= 0; i--)
					{
						if (CraftingRecipe.ItemMatchesForCrafting(items[i], item_index))
						{
							int removed_count = Math.Min(required_count, items[i].Stack);
							required_count -= removed_count;
	...
	public virtual int getCraftableCount(IList<Item> additional_materials)
	{
		...
		foreach (KeyValuePair<string, int> pair in this.recipeList)
		{
			...
			{
				if (Game1.player.Items[i] is Object obj2 && (obj2.QualifiedItemId == itemId || obj2.Category.ToString() == itemId || CraftingRecipe.isThereSpecialIngredientRule(obj2, itemId)))
				{
			...
			if (additional_materials != null)
			{
				...
				{
					if (additional_materials[c] is Object obj && (obj.QualifiedItemId == itemId || obj.Category.ToString() == itemId || CraftingRecipe.isThereSpecialIngredientRule(obj, itemId)))
					{
