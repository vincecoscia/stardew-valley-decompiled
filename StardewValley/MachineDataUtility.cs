...
public static class MachineDataUtility
{
	...
	public static Item GetOutputItem(Object machine, MachineItemOutput outputData, Item inputItem, Farmer who, bool probe, out int? overrideMinutesUntilReady)
	{
		...
		{
			if (!StaticDelegateBuilder.TryCreateDelegate<MachineOutputDelegate>(outputData.OutputMethod, out var method, out var error2))
			{
				Game1.log.Warn($"Machine {machine.QualifiedItemId} has invalid item output method '{outputData.OutputMethod}': {error2}");
				return null;
			}
		...
		{
			item = ItemQueryResolver.TryResolveRandomItem(outputData, context, avoidRepeat: false, null, (string id) => MachineDataUtility.FormatOutputId(id, machine, outputData, inputItem, who), inputItem, delegate(string query, string error)
			{
