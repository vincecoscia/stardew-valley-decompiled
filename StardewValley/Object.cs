...
public class Object : Item
{
	...
	public override string Name
	{
		get
		{
			return this.name + (base.isRecipe ? " Recipe" : "");
		}
		set
		{
			this.name = value;
	...
	protected override void initNetFields()
	{
		...
		this.signText.FilterStringEvent += Utility.FilterDirtyWords;
	}
	...
	protected virtual string loadDisplayName()
	{
		...
		string preservedName = ((this.preservedParentSheetIndex.Value != null) ? ItemRegistry.GetDataOrErrorItem("(O)" + this.preservedParentSheetIndex).DisplayName : null);
		if (this.displayNameFormat != null)
		{
			string result = TokenParser.ParseText(this.displayNameFormat);
			if (result.Contains('%'))
			{
	...
	public virtual bool performToolAction(Tool t)
	{
		...
		if (base.QualifiedItemId == "(BC)165" && this.heldObject.Value is Chest chest2 && !chest2.isEmpty())
		{
			chest2.clearNulls();
		...
		else
		{
			...
			{
				this.MinutesUntilReady -= (int)t.upgradeLevel + 1;
				if (this.MinutesUntilReady <= 0)
				{
					this.fragility.Value = 2;
		...
		if (base.QualifiedItemId == "(O)590" || base.QualifiedItemId == "(O)SeedSpot")
		{
			if (t is Hoe)
			{
				...
				if (!location.terrainFeatures.ContainsKey(this.tileLocation.Value))
				{
		...
		if ((bool)this.bigCraftable && !(t is MeleeWeapon) && t.isHeavyHitter() && ItemRegistry.GetDataOrErrorItem(base.QualifiedItemId).IsErrorItem)
		{
			this.playNearbySoundAll("hammer");
		...
		if (this.Type == "Crafting" && !(t is MeleeWeapon) && t.isHeavyHitter())
		{
			...
			if (base.QualifiedItemId == "(BC)21" && this.heldObject.Value != null)
			{
				location.debris.Add(new Debris(this.heldObject.Value, this.tileLocation.Value * 64f + new Vector2(32f, 32f)));
				this.heldObject.Value = null;
			}
			...
			if (this.heldObject.Value != null && (bool)this.readyForHarvest)
			{
				location.debris.Add(new Debris(this.heldObject.Value, this.tileLocation.Value * 64f + new Vector2(32f, 32f)));
			}
	...
	{
		if (!this.bigCraftable && this.type != null && this.Type != "Quest" && this.canBeTrashed() && !(this is Furniture))
		{
	...
	public virtual void DayUpdate()
	{
		GameLocation location = this.Location;
		this.health = 10;
		if (this.IsSprinkler() && (!location.isOutdoors || !location.IsRainingHere()) && this.GetModifiedRadiusForSprinkler() >= 0)
		{
		...
		case "(BC)165":
			...
			{
				if (kvp.Value.GetHarvestType() == FarmAnimalHarvestType.HarvestWithTool && kvp.Value.currentProduce.Value != null && this.heldObject.Value is Chest chest)
				{
		...
		if ((bool)this.bigCraftable && this.name.Contains("Seasonal"))
		{
	...
	public override void actionWhenBeingHeld(Farmer who)
	{
		...
		if (location != null)
		{
			...
			{
				if (this.lightSource != null && location.hasLightSource((int)who.UniqueMultiplayerID))
				{
			...
			if (this.lightSource != null && (!this.bigCraftable || (bool)this.isLamp))
			{
				if (!location.hasLightSource((int)who.UniqueMultiplayerID))
				{
					location.sharedLights[(int)who.UniqueMultiplayerID] = new LightSource(this.lightSource.textureIndex, this.lightSource.position.Value, this.lightSource.radius.Value, this.lightSource.color.Value, (int)who.UniqueMultiplayerID, LightSource.LightContext.None, who.UniqueMultiplayerID);
				}
	...
	{
		GameLocation location = who.currentLocation;
		if (this.lightSource != null && location != null && location.hasLightSource((int)who.UniqueMultiplayerID))
		{
			location.removeLightSource((int)who.UniqueMultiplayerID);
		}
	...
	public virtual bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
	{
		...
		if (this.IsSprinkler())
		{
			...
			if (dropIn.QualifiedItemId == "(O)93" && base.SpecialVariable == 0)
			{
				if (probe)
				{
					return true;
				...
				this.lightSource = new LightSource(4, new Vector2(this.tileLocation.X * 64f + 16f, this.tileLocation.Y * 64f + 16f), 1.25f, new Color(1, 1, 1) * 0.9f, (int)(this.tileLocation.X * 2000f + this.tileLocation.Y), LightSource.LightContext.None, 0L);
				return true;
			}
		}
		...
		MachineData machineData = this.GetMachineData();
		if (this.heldObject.Value != null && machineData != null && !machineData.AllowLoadWhenFull)
		{
			return false;
		}
	...
	public virtual bool OutputMachine(MachineData machine, MachineOutputRule outputRule, Item inputItem, Farmer who, GameLocation location, bool probe)
	{
		...
		if (this.heldObject.Value != null && !machine.AllowLoadWhenFull)
		{
			return false;
		}
	...
	public virtual void updateWhenCurrentLocation(GameTime time)
	{
		...
		if ((bool)this.readyForHarvest && !this._hasHeldObject)
		{
			this.readyForHarvest.Value = false;
		}
		if (this._hasLightSource)
		{
			...
			if (light != null && (bool)this.isOn && !environment.hasLightSource(light.Identifier))
			{
		...
		if (this._hasHeldObject)
		{
			...
			if (heldObject._hasLightSource)
			{
				...
				if (this.lightSource != null && !environment.hasLightSource(this.lightSource.Identifier))
				{
					environment.sharedLights[this.lightSource.identifier] = this.lightSource.Clone();
				}
			}
	...
	public virtual void actionOnPlayerEntry()
	{
		...
		{
			this.showNextIndex.Value = (int)this.Location.GetRootLocation().piecesOfHay > 0;
		}
	}

	public override bool canBeTrashed()
	{
		if (!this.questItem && base.QualifiedItemId != "(O)460" && base.QualifiedItemId != "(O)911")
		{
	...
	public virtual void initializeLightSource(Vector2 tileLocation, bool mineShaft = false)
	{
		...
		{
			this.lightSource = new LightSource(4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f - 64f), 2.5f, new Color(0, 80, 160), identifier, LightSource.LightContext.None, 0L);
			return;
		}
		if ((int)furniture?.furniture_type == 16 && (bool)furniture.isOn)
		{
			this.lightSource = new LightSource(4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f - 64f), 1.5f, new Color(0, 80, 160), identifier, LightSource.LightContext.None, 0L);
			return;
		}
		if ((bool)this.bigCraftable)
		{
			if (this is Torch && (bool)this.isOn)
			{
				float y_offset = -64f;
				...
				this.lightSource = new LightSource(4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f + y_offset), 2.5f, new Color(0, 80, 160), identifier, LightSource.LightContext.None, 0L);
				return;
			}
			if ((bool)this.isLamp)
			{
				this.lightSource = new LightSource(4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f - 64f), 3f, new Color(0, 40, 80), identifier, LightSource.LightContext.None, 0L);
				return;
			}
			string qualifiedItemId = base.QualifiedItemId;
			if (qualifiedItemId == "(BC)74")
			{
				this.lightSource = new LightSource(4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f), 1.5f, Color.DarkCyan, identifier, LightSource.LightContext.None, 0L);
				return;
			}
			if (qualifiedItemId == "(BC)96")
			{
				this.lightSource = new LightSource(4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f), 1f, Color.HotPink * 0.75f, identifier, LightSource.LightContext.None, 0L);
				return;
			}
		}
		else if (Utility.IsNormalObjectAtParentSheetIndex(this, base.ItemId) || this is Torch)
		{
			if (base.QualifiedItemId == "(O)95" || ItemContextTagManager.HasBaseTag(base.QualifiedItemId, "torch_item"))
			{
				...
				this.lightSource = new LightSource(color: (qualifiedItemId == "94") ? Color.Yellow : ((!(qualifiedItemId == "95")) ? (new Color(1, 1, 1) * 0.9f) : (new Color(70, 0, 150) * 0.9f)), textureIndex: 4, position: new Vector2(tileLocation.X * 64f + 16f, tileLocation.Y * 64f + 16f), radius: mineShaft ? 1.5f : 1.25f, identifier: identifier, light_context: LightSource.LightContext.None, playerID: 0L);
				return;
			}
			if (base.QualifiedItemId == "(O)746")
			{
				this.lightSource = new LightSource(4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f + 48f), 0.5f, new Color(1, 1, 1) * 0.65f, identifier, LightSource.LightContext.None, 0L);
				return;
			}
			if (this.IsSprinkler() && base.SpecialVariable == 999999)
			{
				this.lightSource = new LightSource(4, new Vector2(tileLocation.X * 64f + 16f, tileLocation.Y * 64f + 16f), 1.25f, new Color(1, 1, 1) * 0.9f, (int)(tileLocation.X * 2000f + tileLocation.Y), LightSource.LightContext.None, 0L);
			}
		}
		if (this.MinutesUntilReady > 0)
		{
			...
			if (light != null)
			{
				this.lightSource = new LightSource(4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f), light.Radius, Utility.StringToColor(light.Color) ?? Color.White, identifier, LightSource.LightContext.None, 0L);
			}
		}
	}
	...
	public virtual void performRemoveAction()
	{
		...
		{
			string id = (((int)base.quality != 0) ? ((int)base.quality - 1).ToString() : this.preservedParentSheetIndex.Value);
			if (id != null)
			{
		...
		if (this.name != null && this.name.Contains("Seasonal") && this.bigCraftable.Value)
		{
			base.ResetParentSheetIndex();
		}
	}
	...
	{
		if ((this.Type == "Crafting" || this.Type == "interactive") && (int)this.fragility != 2)
		{
	...
	public virtual Microsoft.Xna.Framework.Rectangle GetBoundingBoxAt(int x, int y)
	{
		...
		if ((this is Torch && !this.bigCraftable) || base.QualifiedItemId == "(O)590")
		{
	...
	public virtual bool performDropDownAction(Farmer who)
	{
		if (who == null)
		{
			who = Game1.getFarmer(this.owner.Value);
		}
		GameLocation location = this.Location;
		MachineData machineData = this.GetMachineData();
		...
		if (!(qualifiedItemId == "(BC)96"))
		{
			...
			{
				this.showNextIndex.Value = (int)location.GetRootLocation().piecesOfHay >= 0;
			}
		}
		else
		{
		...
		for (int x = (int)center.X - radius; (float)x < center.X + (float)radius; x++)
		{
			for (int y = (int)center.Y - radius; (float)y < center.Y + (float)radius; y++)
			{
				if (Math.Round(Utility.distance(x, center.X, y, center.Y)) == (double)(radius - 1))
				{
					...
					{
						if (this.name.Equals("Forest") && x >= 93 && y <= 22)
						{
							continue;
						}
	...
	private void readBook(GameLocation location)
	{
		...
		if (Game1.player.stats.Get(base.itemId) != 0 && base.ItemId != "Book_PriceCatalogue" && base.ItemId != "Book_AnimalCatalogue")
		{
			...
			bool foundAny = false;
			foreach (string s in base.GetContextTags())
			{
		...
		if (!(text == "Book_QueenOfSauce"))
		{
			...
			Game1.player.stats.Increment(base.itemId);
			DelayedAction.functionAfterDelay(delegate
			{
		...
		int num = 0;
		foreach (KeyValuePair<string, string> s2 in dictionary)
		{
			if (!Game1.player.cookingRecipes.ContainsKey(s2.Value.Split("/")[0]))
			{
				Game1.player.cookingRecipes.Add(s2.Value.Split("/")[0], 0);
				num++;
			}
		}
		Game1.player.stats.Increment(base.itemId);
	...
	public virtual bool checkForAction(Farmer who, bool justCheckingForActivity = false)
	{
		...
		case "(BC)TextSign":
			return this.CheckForActionOnTextSign(who, justCheckingForActivity);
		case "(O)464":
	...
	protected bool CheckForActionOnScarecrow(Farmer who, bool justCheckingForActivity = false)
	{
		...
		{
			this.shakeTimer = 100;
			if ((int)base.quality != 0)
			{
				Game1.createItemDebris(ItemRegistry.Create("(H)" + ((int)base.quality - 1)), this.tileLocation.Value * 64f, (who.FacingDirection + 2) % 4);
				base.quality.Value = 0;
			}
			if (this.preservedParentSheetIndex.Value != null)
			{
	...
	protected virtual bool CheckForActionOnTextSign(Farmer who, bool justCheckingForActivity = false)
	{
		...
		{
			TitleTextInputMenu signMenu = new TitleTextInputMenu(Game1.content.LoadString("Strings\\UI:TextSignEntry"), null, this.signText.Value, null);
			...
			signMenu.doneNaming = delegate(string text)
			{
				...
				if (!string.IsNullOrEmpty(this.signText.Value))
				{
					this.showNextIndex.Value = false;
				}
				else
	...
	protected bool CheckForActionOnMachine(Farmer who, bool justCheckingForActivity = false)
	{
		...
		if ((bool)this.readyForHarvest)
		{
			...
			if (this.lastOutputRuleId.Value != null)
			{
				...
				{
					Object prevOutput = this.heldObject.Value;
					this.heldObject.Value = null;
					this.OutputMachine(machineData, outputRule, this.lastInputItem.Value, who, location, probe: false);
					if (this.heldObject.Value == null)
					{
						this.heldObject.Value = prevOutput;
					}
				}
			}
			bool check_for_reload = false;
			Object objectThatWasHeld = this.heldObject.Value;
			if (who.IsLocalPlayer)
			{
				this.heldObject.Value = null;
				if (!who.addItemToInventoryBool(objectThatWasHeld))
				{
					this.heldObject.Value = objectThatWasHeld;
					Game1.showRedMessage(Game1.content.LoadString("Strings\\StringsFromCSFiles:Crop.cs.588"));
					return false;
				}
		...
		{
			if (StaticDelegateBuilder.TryCreateDelegate<MachineInteractDelegate>(machineData2.InteractMethod, out var method, out var error))
			{
				if (!justCheckingForActivity)
				{
			...
			Game1.log.Warn($"Machine {base.ItemId} has invalid interaction method '{machineData2.InteractMethod}': {error}");
		}
		return false;
	}
	...
	{
		foreach (string context_tag in base.GetContextTags())
		{
	...
	public virtual void farmerAdjacentAction(Farmer who, bool diagonal = false)
	{
		...
		case "(O)463":
			if ((this.internalSound == null || (Game1.currentGameTime.TotalGameTime.TotalMilliseconds - (double)this.lastNoteBlockSoundTime >= 1000.0 && !this.internalSound.IsPlaying)) && !Game1.dialogueUp && !diagonal)
			{
				...
				Game1.playSound("drumkit" + preservedParentSheetInt2, out this.internalSound);
				this.scale.Y = 1.3f;
				this.shakeTimer = 200;
				this.lastNoteBlockSoundTime = (int)Game1.currentGameTime.TotalGameTime.TotalMilliseconds;
			}
		...
		case "(BC)29":
		{
			...
			foreach (Vector2 offset in adjacentTilesOffsets)
			{
				...
				if (!location.IsTileOccupiedBy(v2) && location.isTilePassable(new Location((int)v2.X, (int)v2.Y), Game1.viewport) && location.isCharacterAtTile(v2) == null)
				{
					...
					Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(352, 400f, 2, 1, v2 * new Vector2(64f, 64f), flicker: false, flipped: false));
		...
		if (!diagonal)
		{
			...
			if (this.Location.objects.ContainsKey(v) && this.Location.objects[v].ItemId == "TextSign")
			{
				this.Location.objects[v].hovering = true;
			}
		}
	}
	...
	public virtual bool minutesElapsed(int minutes)
	{
		...
		if (this.heldObject.Value != null && base.QualifiedItemId != "(BC)165")
		{
			...
			if (this.MinutesUntilReady <= 0 && (machineData == null || !machineData.IsIncubator))
			{
				if (!this.readyForHarvest && (!Game1.newDaySync.hasInstance() || Game1.newDaySync.hasFinished()))
				{
			...
			if (machineData != null)
			{
				if (!this.readyForHarvest && machineData.WorkingEffects != null && Game1.random.NextDouble() < (double)machineData.WorkingEffectChance)
				{
					this.addWorkingAnimation();
				}
			}
			else if (!this.readyForHarvest && Game1.random.NextDouble() < 0.33)
			{
				this.addWorkingAnimation();
			}
		}
		else if ((bool)this.bigCraftable)
		{
			switch (base.QualifiedItemId)
			{
			case "(BC)29":
			...
			case "(BC)83":
				this.showNextIndex.Value = false;
				environment.removeLightSource((int)(this.tileLocation.X * 797f + this.tileLocation.Y * 13f + 666f));
				break;
			}
		}
		return false;
	...
	{
		if (!this.bigCraftable && this.Type == "Arch")
		{
	...
	{
		if (this.type == null || this.Type == "Arch" || (bool)this.bigCraftable)
		{
			return false;
		}
	...
	public override IEnumerable<Buff> GetFoodOrDrinkBuffs()
	{
		...
		if ((int)this.edibility <= -300 || !Game1.objectData.TryGetValue(base.ItemId, out var data))
		{
			yield break;
		}
	...
	public virtual bool ShouldWobble()
	{
		if (this.minutesUntilReady.Value > 0 && !this.readyForHarvest.Value)
		{
			...
			if (machineData != null)
			{
				return machineData.WobbleWhileWorking;
			}
			if (this.bigCraftable.Value)
			{
				switch (base.QualifiedItemId)
	...
	public virtual void drawPlacementBounds(SpriteBatch spriteBatch, GameLocation location)
	{
		...
		if ((bool)this.bigCraftable || this is Furniture || ((int)base.category != -74 && (int)base.category != -19))
		{
	...
	public virtual void drawAsProp(SpriteBatch b)
	{
		...
		else
		{
			...
			b.Draw(itemData2.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, y * 64 + 32)), itemData2.GetSourceRect(), Color.White, 0f, new Vector2(8f, 8f), (this.scale.Y > 1f) ? this.getScale().Y : 4f, this.flipped ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
		}
	}
	...
	public virtual void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if (this.hovering)
		{
			if (base.ItemId == "TextSign" && this.signText.Value != null && this.signText.Value.Length > 0)
			{
				Vector2 position = Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, y * 64 - 64));
				SpriteText.drawSmallTextBubble(spriteBatch, this.signText.Value, position, 256, 0.98f + this.TileLocation.X * 0.0001f + this.TileLocation.Y * 1E-06f);
			}
			this.hovering = false;
		}
		if ((bool)this.bigCraftable)
		{
			...
			if (this.heldObject.Value != null)
			{
				...
				{
					offset2 = FarmAnimal.GetAnimalDataFromEgg(this.heldObject.Value, location)?.IncubatorParentSheetOffset ?? 1;
				}
			}
			if (this._machineAnimationFrame >= 0 && this._machineAnimation != null)
			{
				offset2 = this._machineAnimationFrame;
			}
			...
			if (base.QualifiedItemId == "(BC)272")
			{
				...
				spriteBatch.Draw(texture, destination, itemData2.GetSourceRect(1, base.ParentSheetIndex), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.None, draw_layer);
				spriteBatch.Draw(texture, position2 + new Vector2(8.5f, 12f) * 4f, itemData2.GetSourceRect(2, base.ParentSheetIndex), Color.White * alpha, (float)Game1.currentGameTime.TotalGameTime.TotalSeconds * -1.5f, new Vector2(7.5f, 15.5f), 4f, SpriteEffects.None, draw_layer + 1E-05f);
				return;
			}
			...
			if ((bool)this.isLamp && Game1.isDarkOut(this.Location))
			{
			...
			if (base.QualifiedItemId == "(BC)126")
			{
				string hatId = (((int)base.quality != 0) ? ((int)base.quality - 1).ToString() : this.preservedParentSheetIndex.Value);
				if (hatId != null)
				{
					...
					spriteBatch.Draw(texture3, position2 + new Vector2(-3f, -6f) * 4f, new Microsoft.Xna.Framework.Rectangle(spriteIndex * 20 % texture3.Width, spriteIndex * 20 / texture3.Width * 20 * 4, 20, 20), (isPrismatic ? Utility.GetPrismaticColor() : Color.White) * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, Math.Max(0f, (float)((y + 1) * 64 - 20) / 10000f) + (float)x * 1E-05f);
				}
			}
		}
		else if (!Game1.eventUp || (Game1.CurrentEvent != null && !Game1.CurrentEvent.isTileWalkedOn(x, y)))
		{
			...
			ParsedItemData itemData = ItemRegistry.GetDataOrErrorItem(base.QualifiedItemId);
			spriteBatch.Draw(itemData.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32 + ((this.shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), y * 64 + 32 + ((this.shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), itemData.GetSourceRect(), Color.White * alpha, 0f, new Vector2(8f, 8f), (this.scale.Y > 1f) ? this.getScale().Y : 4f, this.flipped ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)(this.isPassable() ? bounds.Top : bounds.Bottom) / 10000f);
			if (this.IsSprinkler())
			{
				if (this.heldObject.Value != null)
				{
					...
					spriteBatch.Draw(heldItemData2.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32 + ((this.shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), y * 64 + 32 + ((this.shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0)) + offset), heldItemData2.GetSourceRect(1), Color.White * alpha, 0f, new Vector2(8f, 8f), (this.scale.Y > 1f) ? this.getScale().Y : 4f, this.flipped ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)(this.isPassable() ? bounds.Top : bounds.Bottom) / 10000f + 1E-05f);
				}
		...
		Texture2D texture2 = heldItemData.GetTexture();
		...
		spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, (float)(y * 64 - 64 - 8) + yOffset)), heldItemData.GetSourceRect(), Color.White * 0.75f, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, base_sort + 1E-05f);
	...
	public virtual void draw(SpriteBatch spriteBatch, int xNonTile, int yNonTile, float layerDepth, float alpha = 1f)
	{
		...
		{
			if (base.QualifiedItemId != "(O)590" && base.QualifiedItemId != "(O)SeedSpot" && (int)this.fragility != 2)
			{
	...
	protected override void GetOneCopyFrom(Item source)
	{
		...
		if (source is Object fromObj)
		{
			...
			this.questItem.Value = fromObj.questItem;
			this.questId.Value = fromObj.questId;
	...
	public override bool canBePlacedHere(GameLocation l, Vector2 tile, CollisionMask collisionMask = CollisionMask.All, bool showError = false)
	{
		...
		if (this.IsTapper() && l.terrainFeatures.TryGetValue(tile, out var terrainFeature) && terrainFeature is Tree tree && !l.objects.ContainsKey(tile) && (tree.GetData()?.CanBeTapped() ?? false))
		{
			return true;
		}
		if (base.QualifiedItemId == "(O)805" && l.terrainFeatures.TryGetValue(tile, out var terrainFeature2) && terrainFeature2 is Tree)
		{
			return true;
		}
		...
		if ((int)base.category == -74)
		{
			...
			if (this.IsTeaSapling())
			{
				...
				if (!l.CheckItemPlantRules(base.QualifiedItemId, isFreeGardenPot, (bool)l.isOutdoors || l.IsGreenhouse, out var deniedMessage3))
				{
		...
		if ((int)base.category == -19)
		{
			HoeDirt dirt2 = l.GetHoeDirtAtTile(tile);
			if (dirt2 != null && dirt2.CanApplyFertilizer(base.QualifiedItemId))
			{
				if (l.getObjectAtTile((int)tile.X, (int)tile.Y) is IndoorPot pot2)
				{
					return pot2.IsPlantableItem(this);
				}
				return true;
			}
			return false;
	...
	public override bool isPlaceable()
	{
		...
		if (this.type.Value != null && (base.Category == -8 || base.Category == -9 || this.Type == "Crafting" || this.isSapling() || base.QualifiedItemId == "(O)710" || base.Category == -74 || base.Category == -19) && ((int)this.edibility < 0 || this.IsWildTreeSapling()))
		{
			return true;
		}
		return false;
	...
	public virtual bool placementAction(GameLocation location, int x, int y, Farmer who = null)
	{
		...
		if (!this.bigCraftable && !(this is Furniture))
		{
			...
			if (ItemContextTagManager.HasBaseTag(base.QualifiedItemId, "torch_item"))
			{
				...
				location.removeLightSource((int)(this.tileLocation.X * 2000f + this.tileLocation.Y));
		...
		else
		{
			if (this.IsTapper())
			{
				if (location.terrainFeatures.TryGetValue(placementTile, out var terrainFeature5) && terrainFeature5 is Tree tree2 && (int)tree2.growthStage >= 5 && !tree2.stump && !location.objects.ContainsKey(placementTile) && (!tree2.isTemporaryGreenRainTree || Game1.season != Season.Summer))
				{
			...
			case "(BC)275":
			{
				...
				Chest chest2 = new Chest(playerChest: true, placementTile, base.ItemId)
				{
					name = this.name,
					shakeTimer = 50,
		...
		if (base.Category == -19 && location.terrainFeatures.TryGetValue(placementTile, out var terrainFeature3) && terrainFeature3 is HoeDirt { crop: not null } dirt3 && (base.QualifiedItemId == "(O)369" || base.QualifiedItemId == "(O)368") && (int)dirt3.crop.currentPhase != 0)
		{
	...
	{
		if (base.Category == -102 && Game1.player.stats.Get(base.itemId) != 0 && base.ItemId != "Book_PriceCatalogue" && base.ItemId != "Book_AnimalCatalogue")
		{
			foreach (string s in base.GetContextTags())
			{
				if (s.StartsWith("book_xp"))
		...
		if (this.preservedParentSheetIndex.Value != null && (base.QualifiedItemId == "(O)SpecificBait" || base.QualifiedItemId == "(O)DriedFruit" || base.QualifiedItemId == "(O)SmokedFish"))
		{
			text = string.Format(text, ItemRegistry.GetDataOrErrorItem(this.preservedParentSheetIndex.Value).DisplayName);
		}
	...
	public override int sellToStorePrice(long specificPlayerID = -1L)
	{
		...
		if (base.Category == -22)
		{
			return (int)((float)(int)this.price * (1f + (float)(int)base.quality * 0.25f) * (((float)(FishingRod.maxTackleUses - this.uses.Value) + 0f) / (float)FishingRod.maxTackleUses));
		}
		float salePrice = (int)((float)(int)this.price * (1f + (float)base.Quality * 0.25f));
	...
	public override int salePrice(bool ignoreProfitMargins = false)
	{
		...
		default:
		{
			float salePrice = (int)((float)((int)this.price * 2) * (1f + (float)(int)base.quality * 0.25f));
	...
	{
		if ((int)base.category != -74 && !this.isSapling())
		{
	...
	protected virtual float getPriceAfterMultipliers(float startPrice, long specificPlayerID = -1L)
	{
		...
		foreach (Farmer player in Game1.getAllFarmers())
		{
			...
			if (player.professions.Contains(1) && (base.Category == -75 || base.Category == -80 || (base.Category == -79 && !this.isSpawnedObject)))
			{
				multiplier *= 1.1f;
			}
	...
	public override bool ForEachItem(ForEachItemDelegate handler)
	{
		...
		{
			return ForEachItemHelper.ApplyToField(this.heldObject, handler);
		}
		return false;
	}
}
