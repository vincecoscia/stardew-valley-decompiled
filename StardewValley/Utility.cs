...
public class Utility
{
	...
	public static void fixAllAnimals()
	{
		...
		Utility.ForEachLocation(delegate(GameLocation f)
		{
			...
			foreach (Building current in f.buildings)
			{
				if (current.GetIndoors() is AnimalHouse animalHouse)
				{
			...
			foreach (FarmAnimal a2 in allFarmAnimals)
			{
				...
				{
					building.GetIndoors()?.animals.RemoveWhere((KeyValuePair<long, FarmAnimal> pair) => pair.Value.Equals(a2));
				}
				f.animals.RemoveWhere((KeyValuePair<long, FarmAnimal> pair) => pair.Value.Equals(a2));
			}
			foreach (Building b in f.buildings)
			{
				if (b.GetIndoors() is AnimalHouse animalHouse2)
				{
					animalHouse2.animalsThatLiveHere.RemoveWhere((long id) => Utility.getAnimal(id).home != b);
				}
			}
			foreach (FarmAnimal current2 in allFarmAnimals)
			{
				foreach (Building current3 in f.buildings)
				{
					if (current2.CanLiveIn(current3) && current3.GetIndoors() is AnimalHouse animalHouse3 && !animalHouse3.isFull())
					{
						animalHouse3.adoptAnimal(current2);
						break;
					}
				}
			}
			...
			{
				if (current4.home == null)
				{
					current4.Position = Utility.recursiveFindOpenTileForCharacter(current4, f, new Vector2(40f, 40f), 200) * 64f;
					f.animals.TryAdd(current4.myID.Value, current4);
				}
			}
			return true;
		});
		...
		bool ParseWeddingToken(string[] query, out string replacement, Random random, Farmer player)
		{
			...
			case "contextualweddingcelebrations":
			{
				...
				foreach (WeddingAttendeeData attendee2 in contextualAttendees)
				{
					...
					{
						sb2.Append(attendee2.Celebration);
						sb2.Append("/");
					}
				}
	...
	public static Color? StringToColor(string rawColor)
	{
		...
		else if (rawColor.Contains(' '))
		{
			...
			if ((parts.Length == 3 || parts.Length == 4) && ArgUtility.TryGetInt(parts, 0, out var red2, out var error) && ArgUtility.TryGetInt(parts, 1, out var green2, out error) && ArgUtility.TryGetInt(parts, 2, out var blue2, out error) && ArgUtility.TryGetOptionalInt(parts, 3, out var alpha2, out error, 255))
			{
	...
	public static Crop findCloseFlower(GameLocation location, Vector2 startTileLocation, int range = -1, Func<Crop, bool> additional_check = null)
	{
		...
		for (int attempts = 0; range >= 0 || (range < 0 && attempts <= 150); attempts++)
		{
			...
			if (location.terrainFeatures.TryGetValue(currentTile, out var terrainFeature) && terrainFeature is HoeDirt { crop: not null } dirt)
			{
	...
	{
		return Utility.getStandardDescriptionFromItem(item.TypeDefinitionId, item.ItemId, item.isRecipe, item is Ring, stack, delimiter);
	}
	...
	public static void iterateChestsAndStorage(Action<Item> action)
	{
		...
		foreach (Item item4 in Game1.player.team.returnedDonations)
		{
			if (item4 != null)
			{
				action(item4);
		...
		{
			foreach (Item item3 in (IEnumerable<Item>)value)
			{
				if (item3 != null)
				{
					action(item3);
		...
		{
			foreach (Item item2 in specialOrder.donatedItems)
			{
				if (item2 != null)
				{
					action(item2);
	...
	public static bool trySpawnRareObject(Farmer who, Vector2 position, GameLocation location, double chanceModifier = 1.0, double dailyLuckWeight = 1.0, int groundLevel = -1, Random random = null)
	{
		...
		{
			Game1.createItemDebris(Utility.getRandomCosmeticItem(Game1.random), position, -1, location, groundLevel);
		}
	...
	public static bool tryRollMysteryBox(double baseChance, Random r = null)
	{
		...
		baseChance = ((Game1.player.stats.Get("Book_Mystery") == 0) ? (baseChance * 0.75) : (baseChance * 0.95));
	...
	public static Item getTreasureFromGeode(Item geode)
	{
		...
		try
		{
			...
			Random r = Utility.CreateRandom(geodeId.Contains("MysteryBox") ? Game1.stats.Get("MysteryBoxesOpened") : Game1.stats.GeodesCracked, Game1.uniqueIDForThisGame / 2);
