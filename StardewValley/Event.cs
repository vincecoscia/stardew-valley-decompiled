...
public class Event
{
	...
	public static class DefaultCommands
	{
		...
		public static void Move(Event @event, string[] args, EventContext context)
		{
			...
			{
				if (!ArgUtility.TryGetOptionalBool(args, args.Length - 1, out var rawValue, out var error2))
				{
		...
		public static void WarpFarmers(Event @event, string[] args, EventContext context)
		{
			...
			{
				if (!ArgUtility.TryGetPoint(args, j, out var position, out error) || !ArgUtility.TryGetDirection(args, j + 2, out var facingDirection, out error))
				{
					context.LogErrorAndSkip(error);
					return;
				}
		...
		public static void ShowFrame(Event @event, string[] args, EventContext context)
		{
			...
			{
				actorName = "farmer";
				if (!ArgUtility.TryGetInt(args, 1, out frame, out var error))
				{
					context.LogErrorAndSkip(error);
					return;
				}
			}
			else if (!ArgUtility.TryGet(args, 1, out actorName, out error2) || !ArgUtility.TryGetInt(args, 2, out frame, out error2) || !ArgUtility.TryGetOptionalBool(args, 3, out flip, out error2))
			{
				context.LogErrorAndSkip(error2);
				return;
			}
		...
		public static void TemporaryAnimatedSprite(Event @event, string[] args, EventContext context)
		{
			...
			for (int i = 19; i < args.Length; i++)
			{
				...
				case "motion":
				{
					if (!ArgUtility.TryGetVector2(args, i + 1, out var value3, out error))
					{
						context.LogError(error);
						break;
					}
				...
				case "acceleration":
				{
					if (!ArgUtility.TryGetVector2(args, i + 1, out var value2, out error))
					{
						context.LogError(error);
						break;
					}
		...
		public static void Fork(Event @event, string[] args, EventContext context)
		{
			...
			if (isTranslationKey)
			{
				...
				commands = Event.SplitCommands(raw);
			}
			else if (@event.isFestival)
			{
				...
				commands = Event.SplitCommands(raw2);
			}
			else
			{
				...
				commands = Event.SplitCommands(raw3);
			}
		...
		public static void ReplaceWithClone(Event @event, string[] args, EventContext context)
		{
			...
			@event.actors.Add(new NPC(actor.Sprite.Clone(), actor.Position, actor.FacingDirection, actor.Name)
			{
				...
				TemporaryDialogue = actor.CurrentDialogue.DeepClone()
			});
			@event.CurrentCommand++;
		}
			...
			for (int x = 0; x < layer.LayerWidth; x++)
			{
				for (int y = 0; y < layer.LayerHeight; y++)
				{
					if (layer.Tiles[x, y] != null)
					{
						...
						if (actorNamesByIndex.TryGetValue(actorIndex, out var actorName2) && Game1.getCharacterFromName(actorName2) != null && (!(actorName2 == "Leo") || Game1.MasterPlayer.mailReceived.Contains("leoMoved")))
						{
							@event.addActor(actorName2, x, y, actorFacingDirection, @event.temporaryLocation);
							npcNames.Add(actorName2);
						}
					}
				}
			}
			if (@event.festivalData != null)
			{
				string keyName = layerId + "_additionalCharacters";
				if (@event.festivalData.TryGetValue(keyName, out var data))
				{
					string[] array = Event.SplitCommands(data);
				...
				foreach (string npcName in npcNames)
				{
					...
					for (int childIndex = 0; childIndex < children.Count; childIndex++)
					{
						...
						for (int j = 0; j < array2.Length; j++)
						{
							...
							pointsToCheck.Add(new Point(spawnPoint.X + offset2.X, spawnPoint.Y + offset2.Y));
						}
				...
				for (int x2 = point.X - clearance; x2 <= point.X + clearance; x2++)
				{
					for (int y2 = point.Y - clearance; y2 <= point.Y + clearance; y2++)
					{
						...
						foreach (NPC actor in @event.actors)
						{
							if (!(actor is Child))
							{
								Point tile2 = actor.TilePoint;
								if (tile2.X == x2 && tile2.Y == y2)
								{
									return false;
								}
							}
		...
		public static void Viewport(Event @event, string[] args, EventContext context)
		{
			...
			else
			{
				...
				string option = null;
				string error2;
				if (!int.TryParse(args[1], out var _) && ArgUtility.TryGet(args, 1, out var NPCTarget, out var error3))
				{
				...
				else if (!ArgUtility.TryGetPoint(args, 1, out position, out error2) || !ArgUtility.TryGetOptional(args, 3, out action, out error2) || !ArgUtility.TryGetOptionalBool(args, (action == "clamp") ? 4 : 3, out shouldFade, out error2) || !ArgUtility.TryGetOptional(args, (action == "clamp") ? 5 : 4, out option, out error2))
				{
					context.LogErrorAndSkip(error2);
					return;
				}
	...
	private static void SetupEventCommandsIfNeeded()
	{
		...
		if (Event.Commands.Count == 0)
		{
			...
			{
				EventCommandDelegate command = (EventCommandDelegate)Delegate.CreateDelegate(typeof(EventCommandDelegate), method4);
				Event.Commands.Add(method4.Name, command);
			}
			array = methods2;
			foreach (MethodInfo method3 in array)
			{
				OtherNamesAttribute attribute2 = method3.GetCustomAttribute<OtherNamesAttribute>();
				if (attribute2 != null)
				{
					string[] aliases = attribute2.Aliases;
					for (int j = 0; j < aliases.Length; j++)
					{
						Event.RegisterCommandAlias(aliases[j], method3.Name);
					}
				}
			}
		}
		...
		{
			EventPreconditionDelegate preconditionDelegate = (EventPreconditionDelegate)Delegate.CreateDelegate(typeof(EventPreconditionDelegate), method2);
			Event.Preconditions[method2.Name] = preconditionDelegate;
		}
		array = methods;
		foreach (MethodInfo method in array)
		{
			...
			if (attribute != null)
			{
				...
				for (int j = 0; j < aliases.Length; j++)
				{
					Event.RegisterPreconditionAlias(aliases[j], method.Name);
				}
			}
		}
	}
	...
	public virtual void tryEventCommand(GameLocation location, GameTime time, string[] args)
	{
		...
		if (Event.CommandAliases.TryGetValue(commandName, out var aliasTarget))
		{
			commandName = aliasTarget;
		}
		if (!Event.Commands.TryGetValue(commandName, out var command))
		{
		...
		: this()
	{
		...
		this.id = eventID;
		eventString = Dialogue.applyGenderSwitchBlocks(farmerActor?.Gender ?? Game1.player.Gender, eventString);
		this.eventCommands = Event.SplitCommands(eventString);
	...
	public static bool tryToLoadFestival(string festival, out Event ev)
	{
		...
		ev.eventCommands = Event.SplitCommands(rawSetUp);
	...
	public void endBehaviors(string[] args, GameLocation location)
	{
		...
		case "newDay":
			...
			this.setExitLocation(Game1.player.homeLocation, (int)Game1.player.mostRecentBed.X / 64, (int)Game1.player.mostRecentBed.Y / 64);
		...
		{
			if (!ArgUtility.TryGet(args, 2, out var npcName4, out var error5, allowBlank: false))
			{
		...
		case "invisible":
		{
			if (!ArgUtility.TryGet(args, 2, out var npcName3, out var error4, allowBlank: false))
			{
				this.LogCommandError(args, error4);
			}
			else if (!this.isMemory)
			{
				NPC npc = Game1.getCharacterFromName(npcName3);
				if (npc == null)
				{
		...
		{
			if (!ArgUtility.TryGet(args, 2, out var npcName2, out var error3, allowBlank: false) || !ArgUtility.TryGet(args, 3, out var dialogue2, out error3))
			{
				this.LogCommandError(args, error3);
				break;
			}
			this.setExitLocation(location.GetFirstPlayerWarp());
			NPC j = Game1.getCharacterFromName(npcName2);
			if (j == null)
			{
		...
		case "dialogue":
		{
			if (!ArgUtility.TryGet(args, 2, out var npcName, out var error2, allowBlank: false) || !ArgUtility.TryGet(args, 3, out var dialogue, out error2))
			{
	...
	public void exitEvent()
	{
		...
		if (this.isFestival)
		{
			...
			foreach (NPC k in this.actors)
			{
				if (k != null)
				{
					this.resetDialogueIfNecessary(k);
				}
			}
			if (Game1.IsMasterGame)
			{
	...
	protected void InitializeEvent(GameLocation location, GameTime time)
	{
		...
		if (!ArgUtility.TryGet(this.eventCommands, 0, out var musicId, out var error) || !ArgUtility.TryGet(this.eventCommands, 1, out var rawCameraPosition, out error, allowBlank: false) || !ArgUtility.TryGet(this.eventCommands, 2, out var rawCharacterPositions, out error, allowBlank: false) || !ArgUtility.TryGetOptional(this.eventCommands, 3, out var rawOption, out error))
		{
		...
		if (!Game1.fadeToBlack || this.actorPositionsAfterMove.Count > 0 || this.CurrentCommand > 3 || this.forked)
		{
			...
			if (this.actorPositionsAfterMove.Count > 0)
			{
				...
				foreach (string s in array)
				{
					...
					foreach (NPC k in this.actors)
					{
						Microsoft.Xna.Framework.Rectangle bounds2 = k.GetBoundingBox();
						if (k.Name.Equals(s) && targetTile.Contains(bounds2) && bounds2.Y - targetTile.Top <= 16)
						{
							k.Halt();
	...
	private void populateWalkLocationsList()
	{
		...
		{
			this.characterWalkLocations.Add(j.Tile);
		}
	...
	private void addActor(string name, int x, int y, int facingDirection, GameLocation location)
	{
		...
		try
		{
			...
			if (this.isFestival)
			{
				try
				{
					if (this.TryGetFestivalDialogueForYear(i, i.Name, out var dialogue))
					{
	...
	private void trySpecialSetUp(GameLocation location)
	{
		...
		case "4324303":
		{
			if (!(location is FarmHouse house5))
			{
				break;
			}
			Point bed_spot2 = house5.GetPlayerBedSpot();
			bed_spot2.X--;
			this.farmer.Position = new Vector2(bed_spot2.X * 64, bed_spot2.Y * 64 + 16);
			this.getActorByName("Penny").setTilePosition(bed_spot2.X - 1, bed_spot2.Y);
			...
			for (int j = 0; j < this.eventCommands.Length; j++)
			{
				...
				if (!ArgUtility.TryGetPoint(args2, 1, out var tile2, out var error2))
				{
				...
				if (location.getObjectAtTile(tile2.X, tile2.Y) == house5.GetPlayerBed())
				{
		...
		case "3912132":
		{
			if (!(location is FarmHouse house6))
			{
				break;
			}
			Point bed_spot = house6.GetPlayerBedSpot();
			bed_spot.X--;
			...
			this.getActorByName("Elliott").setTileLocation(Utility.PointToVector2(bed_spot) + new Vector2(-2f, 0f));
			for (int i = 0; i < this.eventCommands.Length; i++)
			{
				...
				if (location.getObjectAtTile(tile.X, tile.Y) == house6.GetPlayerBed())
				{
	...
	private void addSpecificTemporarySprite(string key, GameLocation location, string[] args)
	{
		...
		case "terraria_warp_begin":
		{
			...
			for (int i12 = 0; i12 < 80; i12++)
			{
				...
				location.TemporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\terraria_cat", new Microsoft.Xna.Framework.Rectangle(113 + Game1.random.Next(3) * 5, 123, 5, 5), 999f, 1, 9999, warpSparklePos, flicker: false, flipped: false, 0.8f, 0.02f, Color.White, 4f, 0f, 0f, 0f)
				{
					...
					scaleChange = (float)Game1.random.Next(-10, 0) / 500f,
					delayBeforeAnimationStart = i12 * 5
				});
			}
			break;
		}
		case "LeoWillyFishing":
		{
			for (int i13 = 0; i13 < 20; i13++)
			{
				...
				{
					layerDepth = (float)(1280 + i13) / 10000f,
					delayBeforeAnimationStart = i13 * 150
				});
			}
			break;
		}
		...
		case "staticSprite":
		{
			if (!ArgUtility.TryGet(args, 2, out var textureName, out var error7) || !ArgUtility.TryGetRectangle(args, 3, out var sourceRect4, out error7) || !ArgUtility.TryGetVector2(args, 7, out var tile2, out error7) || !ArgUtility.TryGetOptionalInt(args, 9, out var id, out error7, 999) || !ArgUtility.TryGetOptionalFloat(args, 10, out var layerDepth, out error7, 1f))
			{
		...
		case "trashBearTown":
			...
			this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(46, 80, 46, 56), new Vector2(43f, 64f) * 64f, flipped: false, 0f, Color.White)
			{
				...
				reachedStopCoordinate = delegate
				{
					...
					this.aboveMapSprites.Add(temporaryAnimatedSprite6);
		...
		case "krobusBeach":
		{
			...
			{
				location.temporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 0, 64, 64), 150f, 4, 0, new Vector2(84f + ((i11 % 2 == 0) ? 0.25f : (-0.05f)), 41f) * 64f, flicker: false, Game1.random.NextBool(), 0.001f, 0.02f, Color.White, 0.75f, 0.003f, 0f, 0f)
				{
					delayBeforeAnimationStart = 500 + i11 * 1000,
					startSound = "waterSlosh"
				});
			}
			this.underwaterSprites = new TemporaryAnimatedSpriteList();
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(82f, 52f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(88f, 52f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(90f, 52f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(79f, 52f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
		...
		{
			if (!MovieTheater.TryGetMovieData("winter_movie_0", out var data3))
			{
				Game1.log.Error("Can't find data for movie 'winter_movie_0'.");
				break;
			}
			Microsoft.Xna.Framework.Rectangle sourceRect3 = MovieTheater.GetSourceRectForScreen(data3.SheetIndex, 9);
			location.temporarySprites.Add(new TemporaryAnimatedSprite
			{
		...
		case "sauceFire":
		{
			...
			for (int i10 = 0; i10 < 8; i10++)
			{
				this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), this.OffsetPosition(new Vector2(64f, 16f) * 64f) + new Vector2(Game1.random.Next(-16, 32), 0f), flipped: false, 0.002f, Color.Gray)
				{
					...
					delayBeforeAnimationStart = i10 * 25
				});
			}
			break;
		}
		...
		case "movieFrame":
		{
			if (!ArgUtility.TryGet(args, 2, out var movieId2, out var error6) || !ArgUtility.TryGetInt(args, 3, out var frame, out error6) || !ArgUtility.TryGetInt(args, 4, out var duration, out error6))
			{
				this.LogCommandError(args, error6);
				break;
			}
			movieId2 = MovieTheater.GetMovieIdFromLegacyIndex(movieId2);
			if (!MovieTheater.TryGetMovieData(movieId2, out var data2))
			{
				this.LogCommandError(args, "no movie found with ID '" + movieId2 + "'");
				break;
			}
			Microsoft.Xna.Framework.Rectangle sourceRect2 = MovieTheater.GetSourceRectForScreen(data2.SheetIndex, frame);
			location.temporarySprites.Add(new TemporaryAnimatedSprite
			{
		...
		{
			if (!ArgUtility.TryGet(args, 2, out var movieId, out var error5) || !ArgUtility.TryGetInt(args, 3, out var screenIndex, out error5) || !ArgUtility.TryGetBool(args, 4, out var shake, out error5))
			{
				this.LogCommandError(args, error5);
				break;
			}
		...
		case "sebastianFrog":
		{
			...
			location.TemporarySprites.Add(new TemporaryAnimatedSprite
			{
				...
				reachedStopCoordinate = delegate
				{
					int num2 = this.CurrentCommand;
					this.CurrentCommand = num2 + 1;
		...
		case "removeSprite":
		{
			if (!ArgUtility.TryGetInt(args, 2, out var spriteId, out var error4))
			{
				this.LogCommandError(args, error4);
			}
			else
		...
		case "arcaneBook":
		{
			...
			{
				location.TemporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(536, 1945, 8, 8), new Vector2(128f, 792f) + new Vector2(Game1.random.Next(32), Game1.random.Next(32) - i8 * 4), flipped: false, 0f, Color.White)
				{
					interval = 50f,
					totalNumberOfLoops = 99999,
					animationLength = 7,
			...
			this.aboveMapSprites = new TemporaryAnimatedSpriteList();
			this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(325, 1977, 18, 18), new Vector2(160f, 800f), flipped: false, 0f, Color.White)
			{
				interval = 25f,
				totalNumberOfLoops = 99999,
				animationLength = 3,
		...
		case "EmilySongBackLights":
		{
			...
			for (int numFlyers = 0; numFlyers < 27; numFlyers++)
			{
				...
				{
					this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(616 + flyerNumber * 10, 1891, 10, 10), 42241f, 1, 1, new Vector2(Game1.graphics.GraphicsDevice.Viewport.Width, yPos3), flicker: false, flipped: false, 0.01f, 0f, Color.White * (1f - (float)tails * 0.11f), 4f, 0f, 0f, 0f)
					{
						yPeriodic = true,
			...
			for (int numRainbows3 = 0; numRainbows3 < 15; numRainbows3++)
			{
				int it = 0;
				int yPos2 = Game1.random.Next(Game1.graphics.GraphicsDevice.Viewport.Width - 128);
				for (int xPos2 = Game1.graphics.GraphicsDevice.Viewport.Height; xPos2 >= -64; xPos2 -= 48)
				{
					this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(597, 1888, 16, 16), 99999f, 1, 99999, new Vector2(yPos2, xPos2), flicker: false, flipped: false, 1f, 0.02f, Color.White, 4f, 0f, -(float)Math.PI / 2f, 0f)
					{
						delayBeforeAnimationStart = 27500 + numRainbows3 * 880 + it * 25,
						local = true
					});
					it++;
				}
			...
			{
				this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(626 + numRainbows2 / 28 * 10, 1891, 10, 10), 2000f, 1, 1, new Vector2(Game1.random.Next(Game1.graphics.GraphicsDevice.Viewport.Width), Game1.random.Next(Game1.graphics.GraphicsDevice.Viewport.Height)), flicker: false, flipped: false, 0.01f, 0f, Color.White, 0.1f, 0f, 0f, 0f)
				{
					motion = new Vector2(0f, -2f),
					alphaFade = 0.002f,
					scaleChange = 0.5f,
		...
		case "parrots1":
			this.aboveMapSprites = new TemporaryAnimatedSpriteList();
			this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(0, 165, 24, 22), 100f, 6, 9999, new Vector2(Game1.graphics.GraphicsDevice.Viewport.Width, 256f), flicker: false, flipped: false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
			...
			this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(0, 165, 24, 22), 100f, 6, 9999, new Vector2(Game1.graphics.GraphicsDevice.Viewport.Width, 192f), flicker: false, flipped: false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(-3f, 0f),
				yPeriodic = true,
				yPeriodicLoopTime = 2000f,
			...
			this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(0, 165, 24, 22), 100f, 6, 9999, new Vector2(Game1.graphics.GraphicsDevice.Viewport.Width, 320f), flicker: false, flipped: false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(-3f, 0f),
				yPeriodic = true,
				yPeriodicLoopTime = 2000f,
		...
		case "EmilySign":
		{
			...
			for (int numRainbows = 0; numRainbows < 10; numRainbows++)
			{
				...
				{
					this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(597, 1888, 16, 16), 99999f, 1, 99999, new Vector2(xPos, yPos), flicker: false, flipped: false, 1f, 0.02f, Color.White, 4f, 0f, 0f, 0f)
					{
		...
		case "getEndSlideshow":
		{
			...
			string[] s = Event.SplitCommands(obj.getEndSlideshow());
		...
		case "krobusraven":
		{
			...
			for (int i6 = 0; i6 < 12; i6++)
			{
				location.TemporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(16, 594, 16, 12), 100f, 2, 999999, new Vector2(Game1.viewport.Width, (float)Game1.viewport.Height * 0.33f + (float)Game1.random.Next(-128, 128)), flicker: false, flipped: false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
					...
					yPeriodicRange = 32f,
					local = true,
					delayBeforeAnimationStart = 24000 + i6 * 200,
					startSound = ((i6 == 0) ? "yoba" : null)
				});
			}
			int whenToStart = 0;
		...
		case "wed":
		{
			...
			{
				Vector2 position4 = new Vector2(Game1.random.Next(Game1.viewport.Width - 128), Game1.random.Next(Game1.viewport.Height));
				int scale = Game1.random.Next(2, 5);
				this.aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(424, 1266, 8, 8), 60f + (float)Game1.random.Next(-10, 10), 7, 999999, position4, flicker: false, flipped: false, 0.99f, 0f, Color.White, scale, 0f, 0f, 0f)
				{
					local = true,
		...
		case "jojaCeremony":
		{
			...
			{
				Vector2 position3 = new Vector2(Game1.random.Next(Game1.viewport.Width - 128), Game1.viewport.Height + i2 * 64);
		...
		case "ccCelebration":
		{
			...
			for (int n = 0; n < 32; n++)
			{
				Vector2 position2 = new Vector2(Game1.random.Next(Game1.viewport.Width - 128), Game1.viewport.Height + n * 64);
		...
		case "moonlightJellies":
			...
			this.underwaterSprites = new TemporaryAnimatedSpriteList();
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(26f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(29f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(31f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(20f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(17f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(16f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(12f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(9f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(18f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(33f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(36f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(304, 16, 16, 16), 200f, 3, 9999, new Vector2(21f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(304, 16, 16, 16), 200f, 3, 9999, new Vector2(20f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1.5f),
				xPeriodic = true,
				xPeriodicLoopTime = 2500f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(304, 16, 16, 16), 200f, 3, 9999, new Vector2(22f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1.5f),
				xPeriodic = true,
				xPeriodicLoopTime = 2500f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(304, 16, 16, 16), 200f, 3, 9999, new Vector2(33f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1.5f),
				xPeriodic = true,
				xPeriodicLoopTime = 2500f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(304, 16, 16, 16), 200f, 3, 9999, new Vector2(36f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1.5f),
				xPeriodic = true,
				xPeriodicLoopTime = 2500f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 32, 16, 16), 250f, 3, 9999, new Vector2(28f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(40f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(42f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(43f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(45f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(46f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(48f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(49f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(50f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(51f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(52f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(53f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(54f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(55f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(4f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(5f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(3f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(6f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(8f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1f),
				xPeriodic = true,
				xPeriodicLoopTime = 3000f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(304, 16, 16, 16), 200f, 3, 9999, new Vector2(50f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1.5f),
				xPeriodic = true,
				xPeriodicLoopTime = 2500f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(304, 16, 16, 16), 200f, 3, 9999, new Vector2(51f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1.5f),
				xPeriodic = true,
				xPeriodicLoopTime = 2500f,
			...
			this.underwaterSprites.Add(new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(304, 16, 16, 16), 200f, 3, 9999, new Vector2(52f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				motion = new Vector2(0f, -1.5f),
				xPeriodic = true,
				xPeriodicLoopTime = 2500f,
		...
		case "JoshMom":
		{
			...
			for (int m = 0; m < 19; m++)
			{
				location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(516, 1916, 7, 10), 99999f, 1, 99999, new Vector2(64f, 32f), flicker: false, flipped: false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
					...
					motion = new Vector2(-1f, -1f),
					parentSprite = parent,
					delayBeforeAnimationStart = (m + 1) * 1000
				});
			}
			break;
		}
		...
		case "balloonBirds":
		{
			if (!ArgUtility.TryGetOptionalInt(args, 2, out var positionOffset, out var error3))
			{
				this.LogCommandError(args, error3);
				break;
			}
		...
		case "leahPicnic":
		{
			...
			NPC n2 = new NPC(new AnimatedSprite(this.festivalContent, "Characters\\" + (this.farmer.IsMale ? "LeahExMale" : "LeahExFemale"), 0, 16, 32), new Vector2(-100f, -100f) * 64f, 2, "LeahEx");
		...
		case "leahShow":
		{
			...
			NPC n2 = new NPC(new AnimatedSprite(this.festivalContent, "Characters\\" + (this.farmer.IsMale ? "LeahExMale" : "LeahExFemale"), 0, 16, 32), new Vector2(46f, 57f) * 64f, 2, "LeahEx");
			n2.AllowDynamicAppearance = false;
			this.actors.Add(n2);
			break;
		}
		...
		case "heart":
		{
			if (!ArgUtility.TryGetVector2(args, 2, out var tile, out var error2, integerOnly: true))
			{
				this.LogCommandError(args, error2);
				break;
			}
		...
		case "robot":
		{
			TemporaryAnimatedSprite parent2 = new TemporaryAnimatedSprite(this.getActorByName("robot").Sprite.textureName, new Microsoft.Xna.Framework.Rectangle(35, 42, 35, 42), 50f, 1, 9999, new Vector2(13f, 27f) * 64f - new Vector2(0f, 32f), flicker: false, flipped: false, 0.98f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
			...
			for (int l = 0; l < 420; l++)
			{
				location.TemporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(Game1.random.Next(4) * 64, 320, 64, 64), new Vector2(Game1.random.Next(96), 136f), flipped: false, 0.01f, Color.White * 0.75f)
				{
					...
					interval = 9999f,
					motion = new Vector2(Game1.random.Next(-100, 100) / (l + 20), 0.25f + (float)l / 100f),
	...
	public void startSecretSantaEvent()
	{
		...
		this.eventCommands = Event.SplitCommands(rawCommands);
		...
		if (this.festivalTimer > 0)
		{
			...
			if (this.festivalTimer <= 0)
			{
				...
				else
				{
					...
					this.eventCommands = Event.SplitCommands(rawCommands2);
					this.currentCommand = 0;
				}
			}
		}
	...
	public void setUpPlayerControlSequence(string id)
	{
		...
		case "eggHunt":
		{
			...
			for (int x2 = 0; x2 < pathsLayer.LayerWidth; x2++)
			{
				...
				{
					if (pathsLayer.Tiles[x2, y2] != null && pathsLayer.Tiles[x2, y2].TileSheet.Id.StartsWith("fest"))
					{
						this.festivalProps.Add(new Prop(this.festivalTexture, pathsLayer.GetTileIndexAt(x2, y2), 1, 1, 1, x2, y2));
					}
				}
			}
	...
	public void forceFestivalContinue()
	{
		...
		string[] newCommands = Event.SplitCommands(rawCommands);
	...
	/// <summary>Split a raw event script into its component commands.</summary>
	/// <param name="rawScript">The raw event script to split.</param>
	public static string[] SplitCommands(string rawScript)
	{
	...
	public void setUpFestivalMainEvent()
	{
		...
		{
			string female3 = leftoverFemales.Last();
			if (leftoverMales.Contains(Utility.getLoveInterest(female3)))
			{
				females.Add(new NetDancePartner(female3));
				males.Add(new NetDancePartner(Utility.getLoveInterest(female3)));
			}
			leftoverFemales.Remove(female3);
		}
		...
		for (int l = 1; l <= 6; l++)
		{
			...
			string male2 = ((!males[l - 1].IsVillager()) ? ("farmer" + Utility.getFarmerNumberFromFarmer(males[l - 1].TryGetFarmer())) : males[l - 1].TryGetVillager().Name);
			rawFestivalData = rawFestivalData.Replace("Girl" + l, female2);
			rawFestivalData = rawFestivalData.Replace("Guy" + l, male2);
		}
		...
		{
			NetDancePartner female = females[k];
			NetDancePartner male = males[k];
		...
		List<string> commandsToAdd = new List<string>(Event.SplitCommands(rawFestivalData));
		...
		this.eventCommands = Event.SplitCommands(rawFestivalData);
	}
	...
	private void initiateGrangeJudging()
	{
		...
		if (this.getActorByName("Marnie") != null)
		{
			for (int i = this.npcControllers.Count - 1; i >= 0; i--)
			{
				if (this.npcControllers[i].puppet.Name.Equals("Marnie"))
				{
					this.npcControllers.RemoveAt(i);
				}
			}
	...
	public bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
	{
		if (this.isFestival)
		{
			...
			{
			case 958:
				if (this.isSpecificFestival("fall27") && (tileLocation.X == 61 || tileLocation.X == 44) && (tileLocation.Y == 13 || tileLocation.Y == 9) && who.IsLocalPlayer)
				{
			...
			case 175:
			case 176:
				if (this.isSpecificFestival("fall16") && isMainFestivalTilesheet && who.IsLocalPlayer)
				{
			...
			case 308:
			case 309:
				if (this.isSpecificFestival("fall16") && isMainFestivalTilesheet)
				{
					...
					if (who.IsLocalPlayer && this.isSpecificFestival("fall16"))
					{
			...
			case 510:
			case 511:
				if (this.isSpecificFestival("fall16") && isMainFestivalTilesheet && who.IsLocalPlayer && this.isSpecificFestival("fall16"))
				{
			...
			case 506:
				if (this.isSpecificFestival("fall16") && isMainFestivalTilesheet && who.IsLocalPlayer)
				{
					if (who.Money >= 100 && !who.mailReceived.Contains("fortuneTeller" + Game1.year))
					{
						...
						location.createQuestionDialogue(Game1.parseText(Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1691")), responses3, "fortuneTeller");
					}
			...
			if (tileAction != null)
			{
				try
				{
					...
					case "Shop":
					{
						if (!ArgUtility.TryGet(args, 1, out var shop_id, out var error3))
						{
						...
						if (this.festivalData.TryGetValue(shop_id, out var legacyShopData))
						{
							...
							if (!this.festivalShops.TryGetValue(shop_id, out var stockList))
							{
								...
								{
									if (!ArgUtility.TryGet(args, i, out var type, out error3) || !ArgUtility.TryGet(args, i + 1, out var itemId, out error3) || !ArgUtility.TryGetInt(args, i + 2, out var price, out error3) || !ArgUtility.TryGetInt(args, i + 3, out var stock, out error3))
									{
										Game1.log.Error($"Festival '{this.id}' has legacy shop inventory '{string.Join(" ", inventoryList)}' which couldn't be parsed: {error3}.");
										break;
									}
						...
						if (!opened && Utility.TryOpenShopMenu(shop_id, this.temporaryLocation, null, null, forceOpen: false, playOpenSound: true, delegate(string message)
						{
					...
					case "Message":
					{
						if (!ArgUtility.TryGet(args, 1, out var translationKey, out var error2))
						{
			...
			else if (this.isFestival)
			{
				if (who.IsLocalPlayer && (!this.playerControlSequence || !this.playerControlSequenceID.Equals("iceFishing")))
				{
					foreach (NPC k in this.actors)
					{
						...
						_ = k.name == "Penny";
						if (((tile2.X != tileLocation.X || (tile2.Y != tileLocation.Y && tile2.Y != tileLocation.Y + 1)) && !k.GetBoundingBox().Intersects(tileRect)) || (k.CurrentDialogue.Count < 1 && (k.CurrentDialogue.Count <= 0 || k.CurrentDialogue.Peek().isOnFinalDialogue()) && !k.Equals(this.festivalHost) && (!k.datable || !this.isSpecificFestival("spring24")) && (this.secretSantaRecipient == null || !k.Name.Equals(this.secretSantaRecipient.Name))))
						{
							continue;
						}
						Friendship friendship;
						bool divorced = who.friendshipData.TryGetValue(k.Name, out friendship) && friendship.IsDivorced();
						if ((this.grangeScore > -100 || this.grangeScore == -666) && k.Equals(this.festivalHost) && this.grangeJudged)
						{
							Dialogue message2;
							if (this.grangeScore >= 90)
							{
								Game1.playSound("reward");
								message2 = Dialogue.FromTranslation(k, "Strings\\StringsFromCSFiles:Event.cs.1723", this.grangeScore);
							...
							{
								Game1.playSound("reward");
								message2 = Dialogue.FromTranslation(k, "Strings\\StringsFromCSFiles:Event.cs.1726", this.grangeScore);
								Game1.player.festivalScore += 500;
							}
							else if (this.grangeScore >= 60)
							{
								Game1.playSound("newArtifact");
								message2 = Dialogue.FromTranslation(k, "Strings\\StringsFromCSFiles:Event.cs.1729", this.grangeScore);
							...
							{
								Game1.playSound("newArtifact");
								message2 = Dialogue.FromTranslation(k, "Strings\\StringsFromCSFiles:Event.cs.1732", this.grangeScore);
								Game1.player.festivalScore += 50;
							}
							this.grangeScore = -100;
							k.setNewDialogue(message2);
						}
						else if ((Game1.serverHost == null || Game1.player.Equals(Game1.serverHost.Value)) && k.Equals(this.festivalHost) && (k.CurrentDialogue.Count == 0 || k.CurrentDialogue.Peek().isOnFinalDialogue()) && this.hostMessageKey != null)
						{
							k.setNewDialogue(this.hostMessageKey);
						}
						else if ((Game1.serverHost == null || Game1.player.Equals(Game1.serverHost.Value)) && k.Equals(this.festivalHost) && (k.CurrentDialogue.Count == 0 || k.CurrentDialogue.Peek().isOnFinalDialogue()) && this.hostMessageKey != null)
						{
							k.setNewDialogue(this.hostMessageKey);
						...
						goto IL_12f5;
						IL_12f5:
						if (!divorced && this.secretSantaRecipient != null && k.Name.Equals(this.secretSantaRecipient.Name))
						{
						...
						if (who.spouse != null && k.Name == who.spouse && !this.isSpecificFestival("spring24"))
						{
							Dialogue dialogue2 = null;
							if (k.isRoommate())
							{
								this.TryGetFestivalDialogueForYear(k, k.Name + "_roommate", out dialogue2);
							}
							if (dialogue2 == null)
							{
								this.TryGetFestivalDialogueForYear(k, k.Name + "_spouse", out dialogue2);
							}
							if (dialogue2 != null && (k.CurrentDialogue.Count == 0 || !k.CurrentDialogue.Peek().TranslationKey.Equals(dialogue2.TranslationKey)))
							{
						...
						if (who.dancePartner.Value == null)
						{
							...
							else if (k.CurrentDialogue.Peek().isOnFinalDialogue())
							{
								...
								if (who.spouse != null && k.Name == who.spouse)
								{
									...
									if (k.isRoommate())
									{
										this.TryGetFestivalDialogueForYear(k, k.Name + "_roommate", out dialogue3);
									}
									if (dialogue3 == null)
									{
										this.TryGetFestivalDialogueForYear(k, k.Name + "_spouse", out dialogue3);
									}
									if (dialogue3 != null)
									{
										k.CurrentDialogue.Clear();
										k.CurrentDialogue.Push(dialogue3);
				...
				if (this.festivalData != null && this.isSpecificFestival("spring13"))
				{
					...
					for (int j = this.festivalProps.Count - 1; j >= 0; j--)
					{
						if (this.festivalProps[j].isColliding(tile))
						{
							who.festivalScore++;
							this.festivalProps.RemoveAt(j);
	...
	{
		for (int i = this.festivalProps.Count - 1; i >= 0; i--)
		{
			if (this.festivalProps[i].isColliding(rect))
			{
				this.festivalProps.RemoveAt(i);
			}
		}
	}
	...
	public void answerDialogue(string questionKey, int answerChoice)
	{
		...
		case "fishingGame":
			...
			break;
		case "starTokenShop":
			if (answerChoice == 0 && Utility.TryOpenShopMenu("Festival_StardewValleyFair_StarTokens", this.temporaryLocation, null, null, forceOpen: false, playOpenSound: false) && Game1.activeClickableMenu is ShopMenu shop)
			{
	...
	public void setUpAdvancedMove(string[] args, NPCController.endBehavior endBehavior = null)
	{
		...
		{
			NPC j = this.getActorByName(actorName, legacyReplaceUnderscores: true);
			if (j != null)
			{
	...
	private void eggHuntWinner()
	{
		...
		foreach (Farmer temp in Game1.getOnlineFarmers())
		{
			...
			{
				winners.Add(temp);
				this.festivalWinners.Add(temp.UniqueMultiplayerID);
			}
		}
	...
	private void iceFishingWinner()
	{
		...
		if (mostFishScore >= numberOfFishToWin)
		{
			...
			{
				winnerDialogue = Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1864");
				for (int i = 0; i < this.iceFishWinners.Count; i++)
				{
					if (i == this.iceFishWinners.Count - 1)
					{
						winnerDialogue += Game1.content.LoadString("Strings\\StringsFromCSFiles:Event.cs.1865");
					}
					winnerDialogue = winnerDialogue + " " + this.iceFishWinners[i].displayName;
					if (i < this.iceFishWinners.Count - 1)
					{
						winnerDialogue += ",";
					}
	...
	public void popBalloons(int x, int y)
	{
		...
		for (int j = this.aboveMapSprites.Count - 1; j >= 0; j--)
		{
			TemporaryAnimatedSprite t = this.aboveMapSprites[j];
		...
		foreach (int id in idsToRemove)
		{
			...
			{
				if (this.aboveMapSprites[i].id == id || this.aboveMapSprites[i].id == 9988)
				{
					this.aboveMapSprites.RemoveAt(i);
				}
			}
		}
