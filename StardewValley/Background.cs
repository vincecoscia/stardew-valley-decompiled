...
public class Background
{
	...
	public virtual void draw(SpriteBatch b)
	{
		if (this.summitBG)
		{
			...
			if (this.cursed && Game1.options.screenFlash)
			{
		...
		if (this.backgroundImage == null)
		{
			...
			{
				if (this.tempSprites[k].update(Game1.currentGameTime))
				{
					this.tempSprites.RemoveAt(k);
				}
			}
			for (int j = 0; j < this.tempSprites.Count; j++)
			{
				this.tempSprites[j].draw(b);
			}
			return;
		}
		Vector2 v = Vector2.Zero;
		Microsoft.Xna.Framework.Rectangle r2 = new Microsoft.Xna.Framework.Rectangle(0, 0, this.chunkWidth, this.chunkHeight);
		for (int l = 0; l < this.chunks.Length; l++)
		{
			v.X = this.position.X + (float)(l * this.chunkWidth % (this.chunksWide * this.chunkWidth)) * this.zoom;
			v.Y = this.position.Y + (float)(l * this.chunkWidth / (this.chunksWide * this.chunkWidth) * this.chunkHeight) * this.zoom;
			if (this.backgroundImage == null)
			{
			...
			r2.X = this.chunks[l] * this.chunkWidth % this.backgroundImage.Width;
			r2.Y = this.chunks[l] * this.chunkWidth / this.backgroundImage.Width * this.chunkHeight;
