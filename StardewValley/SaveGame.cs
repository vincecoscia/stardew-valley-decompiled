...
public class SaveGame
{
	...
	public static IEnumerator<int> getSaveEnumerator()
	{
		...
		saveData.broadcastedMail = new HashSet<string>();
		saveData.broadcastedMail.AddRange(Game1.player.team.broadcastedMail);
		...
		string justFarmerFilePath2 = Path.Combine(save_directory, "SaveGameInfo" + tmpString);
		if (File.Exists(fullFilePath))
		{
			File.Delete(fullFilePath);
		}
		if (save_backups_and_metadata && File.Exists(justFarmerFilePath2))
		{
			File.Delete(justFarmerFilePath2);
		}
		Stream fstream = null;
		try
		{
		...
		SaveGame.serializer.Serialize(writer, saveData);
		...
		if (save_backups_and_metadata)
		{
			...
			try
			{
				fstream = File.Create(justFarmerFilePath2);
			}
			...
			writer.WriteStartDocument();
			SaveGame.farmerSerializer.Serialize(writer, Game1.player);
			writer.WriteEndDocument();
			writer.Flush();
			fstream.Close();
		}
		...
		justFarmerFilePath2 = Path.Combine(save_directory, "SaveGameInfo");
		if (save_backups_and_metadata)
		{
			...
			try
			{
				...
				File.Move(justFarmerFilePath2, justFarmerFilePathOld);
			}
			catch (Exception)
			{
			}
		...
		if (save_backups_and_metadata)
		{
			justFarmerFilePath2 = Path.Combine(save_directory, "SaveGameInfo" + tmpString);
			if (File.Exists(justFarmerFilePath2))
			{
				File.Move(justFarmerFilePath2, justFarmerFilePath2.Replace(tmpString, ""));
			}
		}
		foreach (Farmer allFarmer2 in Game1.getAllFarmers())
		{
			allFarmer2.ApplyAllTrinketEffects();
		}
	...
	{
		string filename = SaveGame.FilterFileName(Game1.GetSaveGameName()) + "_" + Game1.uniqueIDForThisGame + tmpString;
	...
	public static void LoadFarmType()
	{
		...
		if (Game1.whichFarm < 0)
		{
			...
			Game1.log.Warn("Ignored unknown farm type '" + SaveGame.loaded.whichFarm + "' which no longer exists in the data.");
			Game1.whichFarm = 0;
	...
	public static IEnumerator<int> getLoadEnumerator(string file)
	{
		...
		else
		{
			...
			Task deserializeTask2 = new Task(delegate
			{
				Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
				pendingSaveGame = (SaveGame)SaveGame.serializer.Deserialize(stream);
			});
			Game1.hooks.StartTask(deserializeTask2, "Load_Deserialize");
			while (!deserializeTask2.IsCanceled && !deserializeTask2.IsCompleted && !deserializeTask2.IsFaulted)
			{
				yield return 20;
			}
			if (deserializeTask2.IsFaulted)
			{
				Exception e2 = deserializeTask2.Exception.GetBaseException();
				Game1.log.Error("deserializeTask failed with an exception.", e2);
				throw e2;
			}
		...
		else
		{
			...
			Game1.hooks.StartTask(deserializeTask2, "Load_LoadForNewGame");
			while (!deserializeTask2.IsCanceled && !deserializeTask2.IsCompleted && !deserializeTask2.IsFaulted)
			{
				yield return 24;
			}
			if (deserializeTask2.IsFaulted)
			{
				Exception e4 = deserializeTask2.Exception.GetBaseException();
				Game1.log.Error("loadNewGameTask failed with an exception.", e4);
				throw e4;
			}
			if (SaveGame.CancelToTitle)
			{
		...
		if (SaveGame.loaded.bundleData.Count > 0)
		{
			...
			{
				SaveMigrator_1_6.StandardizeBundleFields(SaveGame.loaded.bundleData);
			}
			Game1.netWorldState.Value.SetBundleData(SaveGame.loaded.bundleData);
		...
		if (SaveGame.loaded.locationWeather != null)
		{
			...
			foreach (KeyValuePair<string, LocationWeather> pair2 in SaveGame.loaded.locationWeather)
			{
		...
		{
			foreach (string key4 in SaveGame.loaded.builders.Keys)
			{
				Game1.netWorldState.Value.Builders[key4] = SaveGame.loaded.builders[key4];
			}
		}
		if (LocalMultiplayer.IsLocalMultiplayer())
		{
		...
		else
		{
			...
			Game1.hooks.StartTask(deserializeTask2, "Load_Farmer");
			while (!deserializeTask2.IsCanceled && !deserializeTask2.IsCompleted && !deserializeTask2.IsFaulted)
			{
				yield return 1;
			}
			if (deserializeTask2.IsFaulted)
			{
				Exception e3 = deserializeTask2.Exception.GetBaseException();
				Game1.log.Error("loadFarmerTask failed with an exception", e3);
				throw e3;
			}
		}
		...
		{
			foreach (Farmer farmhand2 in SaveGame.loaded.farmhands)
			{
		...
		foreach (FarmerPair key3 in SaveGame.loaded.farmerFriendships.Keys)
		{
			Game1.player.team.friendshipData[key3] = SaveGame.loaded.farmerFriendships[key3];
		}
		...
		{
			foreach (string key2 in SaveGame.loaded.limitedNutDrops.Keys)
			{
				if (SaveGame.loaded.limitedNutDrops[key2] > 0)
				{
					Game1.player.team.limitedNutDrops[key2] = SaveGame.loaded.limitedNutDrops[key2];
				}
			}
		}
		...
		foreach (SpecialOrder order2 in SaveGame.loaded.specialOrders)
		{
			if (order2 != null)
			{
		...
		foreach (SpecialOrder order in SaveGame.loaded.availableSpecialOrders)
		{
			if (order != null)
			{
		...
		foreach (KeyValuePair<string, List<Item>> pair in SaveGame.loaded.globalInventories)
		{
		...
		foreach (Item donated_item in SaveGame.loaded.returnedDonations)
		{
		...
		else
		{
			...
			Game1.hooks.StartTask(deserializeTask2, "Load_Locations");
			while (!deserializeTask2.IsCanceled && !deserializeTask2.IsCompleted && !deserializeTask2.IsFaulted)
			{
				yield return 1;
			}
			if (deserializeTask2.IsFaulted)
			{
				Exception e = deserializeTask2.Exception.GetBaseException();
				Game1.log.Error("loadLocationsTask failed with an exception", e);
				throw deserializeTask2.Exception.GetBaseException();
			}
		}
	...
	{
		Dictionary<string, string> formerNpcNames = SaveGame.GetFormerNpcNames((string newName, CharacterData _) => Game1.getCharacterFromName(newName) == null);
		...
		foreach (KeyValuePair<string, string> pair in formerNpcNames)
		{
			...
			Game1.log.Debug($"Migrated legacy NPC '{oldName}' in save data to '{newName2}'.");
		}
	}
