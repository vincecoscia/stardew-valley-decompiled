...
public class SerializableDictionary<TKey, TValue> : Dictionary<TKey, TValue>, IXmlSerializable
{
	...
	{
		SerializableDictionary<TKey, TValue>._keySerializer = SaveGame.GetSerializer(typeof(TKey));
		SerializableDictionary<TKey, TValue>._valueSerializer = SaveGame.GetSerializer(typeof(TValue));
	}
	...
	public void ReadXml(XmlReader reader)
	{
		...
		while (reader.NodeType != XmlNodeType.EndElement)
		{
			...
			if (!read)
			{
				key = (TKey)SerializableDictionary<TKey, TValue>._keySerializer.Deserialize(reader);
			}
			reader.ReadEndElement();
			...
			if (!read)
			{
				value = (TValue)SerializableDictionary<TKey, TValue>._valueSerializer.Deserialize(reader);
			}
			reader.ReadEndElement();
			base.Add(key, value);
			reader.ReadEndElement();
	...
	public void WriteXml(XmlWriter writer)
	{
		foreach (TKey key in base.Keys)
		{
			...
			SerializableDictionary<TKey, TValue>._keySerializer.Serialize(writer, key);
			writer.WriteEndElement();
			writer.WriteStartElement("value");
			TValue value = base[key];
			SerializableDictionary<TKey, TValue>._valueSerializer.Serialize(writer, value);
			writer.WriteEndElement();
			writer.WriteEndElement();
		}
	}
