...
public abstract class Tool : Item
{
		...
		get
		{
			...
			if (level >= 1 && level <= 4)
			{
				...
				if (toolData != null && toolData.ApplyUpgradeLevelToDisplayName)
				{
					...
					{
					case 1:
						return Game1.content.LoadString("Strings\\StringsFromCSFiles:Tool.cs.14299", this.displayName);
					case 2:
						return Game1.content.LoadString("Strings\\StringsFromCSFiles:Tool.cs.14300", this.displayName);
					case 3:
						return Game1.content.LoadString("Strings\\StringsFromCSFiles:Tool.cs.14301", this.displayName);
					case 4:
						return Game1.content.LoadString("Strings\\StringsFromCSFiles:Tool.cs.14302", this.displayName);
					}
				}
			}
	...
	public int InitialParentTileIndex
	{
		get
		{
			return this.initialParentTileIndex;
		}
		set
		{
	...
	public int IndexOfMenuItemView
	{
		get
		{
			return this.indexOfMenuItemView;
		}
		set
		{
	...
	{
		if ((int)this.lastUser?.toolPower <= 0 || !this.lastUser.canReleaseTool || !this.lastUser.IsLocalPlayer)
		{
			return;
		}
	...
	public virtual void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
	{
		...
		if (this.isHeavyHitter() && !(this is MeleeWeapon))
		{
			...
			location.damageMonster(new Rectangle(x - 32, y - 32, 64, 64), (int)this.upgradeLevel + 1, ((int)this.upgradeLevel + 1) * 3, isBomb: false, who);
		}
	...
	public virtual bool beginUsing(GameLocation location, int x, int y, Farmer who)
	{
		...
		if (!this.instantUse)
		{
			...
			if ((!(this is FishingRod) && (int)this.upgradeLevel <= 0 && !(this is MeleeWeapon)) || this is Pickaxe)
			{
				who.EndUsingTool();
				return true;
			}
		}
		if ((bool)this.instantUse)
		{
	...
	public virtual void actionWhenClaimed()
	{
		...
		{
			int num = this.indexOfMenuItemView;
			if ((uint)(num - 13) <= 3u)
			{
	...
	{
		if (Game1.player.toolBeingUpgraded.Value == null && (this is Axe || this is Pickaxe || this is Hoe || this is WateringCan || (this is GenericTool && (int)this.indexOfMenuItemView >= 13 && (int)this.indexOfMenuItemView <= 16)))
		{
			return true;
		}
	...
	public virtual bool Forge(Item item, bool count_towards_stats = false)
	{
		...
		else if (item.QualifiedItemId == "(O)852" && this is MeleeWeapon weapon2)
		{
			...
			for (int j = weapon2.enchantments.Count - 1; j >= 0; j--)
			{
				if (weapon2.enchantments[j].IsSecondaryEnchantment() && !(weapon2.enchantments[j] is GalaxySoulEnchantment))
				{
					oldenchantments.Add(weapon2.enchantments[j]);
					weapon2.enchantments.RemoveAt(j);
				}
			}
