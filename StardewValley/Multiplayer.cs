...
public class Multiplayer
{
	...
	public static string protocolVersion => Game1.version + ((Game1.versionLabel != null) ? ("+" + new string(Game1.versionLabel.Where(char.IsLetterOrDigit).ToArray())) : "");
	...
	public virtual void updateRoots()
	{
		...
		if (Game1.IsClient)
		{
			foreach (GameLocation location2 in this.activeLocations())
			{
				if (location2.Root != null && this._updatedRoots.Add(location2.Root.Value))
				{
					...
					this.updateRoot(location2.Root);
				}
			}
		}
		else
	...
	protected virtual void _receivePassoutRequest(Farmer farmer)
	{
		...
		if (lastSleepLocation != null && lastSleepLocation.GetLocationContextId() == farmer.currentLocation.GetLocationContextId() && BedFurniture.IsBedHere(lastSleepLocation, farmer.lastSleepPoint.Value.X, farmer.lastSleepPoint.Value.Y))
		{
			...
			{
				Farmer.performPassoutWarp(farmer, farmer.lastSleepLocation, farmer.lastSleepPoint.Value, has_bed: true);
			}
			return;
		}
	...
	public virtual void processIncomingMessage(IncomingMessage msg)
	{
		...
		case 4:
		{
			...
			if (Game1.getLocationFromName(request.Name)?.findEventById(eventId) == null)
			{
